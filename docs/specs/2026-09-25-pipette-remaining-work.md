...
Christian has authorized implementation with subagents and selected these exact calibration policies: apply and restore in-process through the existing owner; rejecting a run restores its full pre-run calibration even when that replaces later calibration edits. This does not authorize new runtime gates, controller-interlock changes or an unsolicited physical run. The completion evidence below evaluates the delivered work rather than adding runtime prerequisites.
...
6. Implement the approved activation semantics. OEM `adjustZ` and manual paired-tray Set update the in-process table; Christian selected source-equivalent apply/restore through the existing owner, replacing the candidate's next-startup-only behavior. Update the actual consumers consistently without mutating captured sealed XML, restarting, homing, adding another scheduler or inventing new refusal rules.
...
The two calibration policy decisions are now explicit in §1 and PIP-C4. Keep source bugs and any other product differences explicit; this authorization does not permit silently changing scientific behavior or inventing new host gates. Recording a discrepancy is not an instruction to stop normal commands.
