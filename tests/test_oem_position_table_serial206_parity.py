...
def test_duplicate_rows_fail_closed_and_generic_rows_remain_parseable() -> None:
    row = {"name": "LOC_OC", "x": 1, "y": 2, "zLow": 60000, "zDelta": 1000, "inc_factor": 0}
    with pytest.raises(ValueError, match="duplicate"):
        PositionTable.from_rows([row, row])
    generic = PositionTable.from_rows([{**row, "name": "LOC_NOT_REAL"}])
    assert generic.resolve(location_id="LOC_NOT_REAL").location_id == "LOC_NOT_REAL"
