...
def _strict_xy_method_request(*, generation: int = 7, board4: int = 10, board5: int = 11) -> dict:
    return {
        "schema_version": "bioxp.operator_method_request.v1",
        "name": "oem.xy.move_absolute",
        "idempotency_key": f"strict-xy-{generation}-{board4}-{board5}",
        "expected_ownership_generation": generation,
        "expected_board_epoch_by_board": {"4": board4, "5": board5},
...
def test_legacy_command_route_cannot_admit_a_strict_composite_action(tmp_path):
    store = _operator_store(tmp_path)
    with pytest.raises(HTTPException) as excinfo:
        store.admit_command(
            {
                "schema_version": ACTION_REQUEST_SCHEMA,
                "idempotency_key": "legacy-strict-command-1",
    ...
    assert excinfo.value.detail["error"] == "canonical_strict_method_requires_v2_route"
    assert store.connection.execute("SELECT COUNT(*) FROM operator_plane_commands").fetchone()[0] == 0
    store.stop()


def test_generic_method_route_cannot_spoof_strict_method_metadata(tmp_path):
    store = _operator_store(tmp_path)
    with pytest.raises(HTTPException) as excinfo:
        store.admit_method(_strict_xy_method_request(), state=_state())
    assert excinfo.value.detail["error"] == "canonical_strict_method_requires_v2_route"
    assert store.connection.execute("SELECT COUNT(*) FROM operator_plane_methods").fetchone()[0] == 0
    store.stop()


@pytest.mark.parametrize(
    ("method_request", "error"),
    [
        (_strict_xy_method_request(generation=6), "ownership_generation_mismatch"),
        (_strict_xy_method_request(board5=12), "board_epoch_mismatch"),
    ],
)
def test_strict_xy_method_rejects_stale_generation_or_board_epoch(tmp_path, method_request, error):
    store = _operator_store(tmp_path)
    with pytest.raises(HTTPException) as exc:
        store.admit_method(method_request, state=_state(), strict_authority=True)
    assert exc.value.status_code == 409
    assert exc.value.detail["error"] == error
    store.stop()


def test_strict_xy_method_persists_the_validated_board_epoch_fence(tmp_path):
    store = _operator_store(tmp_path)
    admitted = store.admit_method(
        _strict_xy_method_request(), state=_state(), strict_authority=True,
    )
    row = store.connection.execute(
        "SELECT ownership_generation,expected_board_epochs_json "
        "FROM serial206_movement_commands WHERE method_id=?",
        (admitted["method_id"],),
    ).fetchone()
    assert tuple(row) == (7, '{"4":10,"5":11}')
    store.stop()


def _bootstrap_named_store(store: OperatorCommandStore) -> None:
    store.bootstrap_deck_semantic_state({
        ...
        "latch_observation_id": "queue-test-latch",
        "source_operation": "test_bootstrap", "source_command_id": "test-bootstrap",
    })
...
def test_shutdown_retains_lane_owner_when_a_command_worker_does_not_stop(tmp_path) -> None:
    store = _operator_store(tmp_path)
    store.bootstrap_deck_semantic_state({
        "current_location": "LOC_MS", "current_well": 0, "current_tray": None,
        "tip_loaded": False, "tip_dirty": False, "tip_location": -1, "clean_path": True,
        ...
        "ownership_generation": 7, "board_epoch_4": 10, "board_epoch_5": 11,
        "latch_status": True, "machine_latch_closed": True,
        "latch_observation_id": "shutdown-latch",
        "source_operation": "test_bootstrap", "source_command_id": "test-bootstrap",
    })
    admitted = store.admit_command(_request("shutdown-active-deck"), state=_state())
    assert store._acquire_owner() is True
    ...
    authority = DeckAuthoritySnapshot(
        7, "provider-owner", 10, 11, table.digest, 3,
        {"x": 1, "y": 1, "z": 1, "g": 1}, {"global": 0, "x": 0, "y": 0, "z": 0},
        "shutdown-latch", "position-1", 1.0, 0, 0, 65000, "LOC_MS", 0,
        False, False, -1, True, None, 500, "BIOXP", True, True,
    )
    plan = compile_named_location(
        NamedLocationIntent("LOC_OC"), DeckCatalog.from_position_table(table), table, authority
    ...
    store.record_delivery_attempt(
        ...
        work_identity="stage:0:ForceToHighHome",
        plan_digest=plan.plan_digest,
        authority_stamps={"ownership_generation": 7, "board_epoch_4": 10, "board_epoch_5": 11},
    )
    ...
    status_before_stop = store.get_command(admitted["command_id"])["status"]

    with pytest.raises(RuntimeError, match="operator command workers did not stop"):
        store.stop()
    ...
    assert owner == store.owner_id
    assert store.get_command(admitted["command_id"])["status"] == status_before_stop
    time.sleep(0.15)
    ...
    try:
        ...
        worker.alive = False
        guard = getattr(store, "_shutdown_guard_thread", None)
        if guard is not None:
            guard.join(timeout=2.0)
    recovered = store.get_command(admitted["command_id"])
    assert recovered is not None and recovered["status"] == "ambiguous"
    assert store.recovery()["hold"] is True
    store._workers.clear()
    store.connection.close()


def test_shutdown_renews_lane_owner_before_waiting_for_workers(tmp_path) -> None:
    store = _operator_store(tmp_path)
    assert store._acquire_owner() is True

    class Worker:
        alive = True
        lease_during_join = 0.0
        ...
        def join(self, *, timeout: float) -> None:
            assert timeout in {0.25, 1.0}
            self.lease_during_join = float(store.connection.execute(
                "SELECT owner_lease_until FROM operator_plane_lane WHERE singleton=1"
            ).fetchone()[0])
            self.alive = False
    ...
    assert worker.lease_during_join > time.time() + 20.0
    assert store._owner_acquired is False
    store.connection.close()


def test_wp8_background_worker_start_is_atomic_with_worker_registration(tmp_path) -> None:
    store = _operator_store(tmp_path)
    observed: list[bool] = []

    class ProbeThread(threading.Thread):
        def start(self) -> None:
            acquired = store._worker_lock.acquire(blocking=False)
            if acquired:
                store._worker_lock.release()
            observed.append(not acquired)

    worker = ProbeThread()
    store.start_wp8_background_worker(worker, "wp8-command")

    assert observed == [True]
    assert worker in store._workers
    assert store._worker_commands[worker] == "wp8-command"
    store._workers.clear()
    store.connection.close()


def test_command_worker_drain_waits_for_registered_wp8_child(tmp_path) -> None:
    store = _operator_store(tmp_path)
    release = threading.Event()
    worker = threading.Thread(target=lambda: release.wait(1.0), daemon=True)
    store.start_wp8_background_worker(worker, "wp8-command")

    assert store.wait_for_command_workers(["wp8-command"], timeout=0.01) is False
    release.set()
    assert store.wait_for_command_workers(["wp8-command"], timeout=1.0) is True
    store.connection.close()


def test_aggregate_interrupt_includes_live_child_of_terminal_parent(tmp_path) -> None:
    store = _operator_store(tmp_path)
    admitted = store.admit_command(
        {
            "schema_version": ACTION_REQUEST_SCHEMA,
            "idempotency_key": "terminal-parent-live-child",
            "expected_ownership_generation": 7,
    ...
    release = threading.Event()
    worker = threading.Thread(target=lambda: release.wait(1.0), daemon=True)
    store.start_wp8_background_worker(worker, admitted["command_id"])
    store.finish(
        admitted["command_id"], status="completed",
        payload={"ok": True}, claimed=claimed,
    )
    ...
    receipt = store.begin_interrupt(
        ...
        request={
            ...
            "idempotency_key": "interrupt-terminal-parent-live-child",
            "reason": "test",
    ...
    store.clear_interrupt_fence("oem.abort_all")
    store.connection.close()


def test_pending_interrupt_restores_fence_and_releases_it_after_reconciliation(tmp_path) -> None:
    ...
    first.queue_pending_interrupt_reconciliation(
        {
            ...
            "state": _state(),
            "request": {
                "idempotency_key": "restart-pending-aggregate",
                "observed_ownership_generation": 7,
                "observed_board_epoch_by_board": {},
            },
            "interrupt_attempt_id": "restart-pending-aggregate-attempt",
            "attempted": True,
            "acknowledged": True,
            "response": {
                "ok": True,
    ...
    assert second.action_fenced("oem.x.move_steps") is True
    assert second.reconcile_pending_interrupts() == 1
    assert second.action_fenced("oem.x.move_steps") is False
    second.stop()
