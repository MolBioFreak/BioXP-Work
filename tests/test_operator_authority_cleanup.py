...
def _create_legacy_history(path: Path, *, canonical_existing: bool = False) -> None:
    ...
    if canonical_existing:
        ...
        connection.execute(
            """
            INSERT INTO operator_plane_methods(
                method_id,name,source_json,digest,failure_policy,status,version,
                ownership_generation,expanded_count,first_stream_sequence,
                last_stream_sequence,queued_at,updated_at,recovery_outcome_pending
            ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """,
            (
