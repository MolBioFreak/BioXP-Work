...
def test_plate_move_composite_keeps_catch_release_atomic_source_order() -> None:
    ...
    with pytest.raises(RuntimeError, match="source_authority_missing:thermal_door_open"):
        oem_deck_movement.compile_finite_plate_operation(
            "catch_plate", source_leaf_available=True,
            plate=1, plate_location=1, thermal_door_open=None, gripper_version=0,
        )
...
def _bare_wp8_provider():
    ...
    provider.bind_wp8_execution_fence_checker(lambda *_args, **_kwargs: None)
    provider.bind_wp8_background_worker_starter(
        lambda worker, _command_id: worker.start()
    )
    return provider


def test_nested_wp8_plan_rechecks_durable_fence_before_each_physical_child() -> None:
    provider = _bare_wp8_provider()
    delivered: list[str] = []
    boundaries: list[str] = []

    def fence(_command_id: str, *, boundary: str) -> None:
        boundaries.append(boundary)
        if boundary == "before_nested_child_1":
            raise RuntimeError("deck_execution_interrupted")

    provider.bind_wp8_execution_fence_checker(fence)
    provider.execute_wp8_child = lambda child, **_kwargs: (
        delivered.append(str(child["operation"]))
        or {"ok": True, "delivery_attempted": True}
    )
    plan = {
        "plan_digest": "a" * 64,
        "children": [
            {"order": 0, "operation": "first", "arguments": {}},
            {"order": 1, "operation": "second", "arguments": {}},
        ],
    ...
    with pytest.raises(RuntimeError, match="deck_execution_interrupted"):
        provider._wp8_execute_nested_plan(
            plan=plan,
            command_id="nested-command",
            owner_identity={"dispatch_attempt_id": "dispatch-1"},
        )

    assert boundaries == ["before_nested_child_0", "before_nested_child_1"]
    assert delivered == ["first"]


def test_unawaited_wp8_worker_starter_owns_registration_and_thread_start() -> None:
    provider = _bare_wp8_provider()
    release = threading.Event()
    started_threads: list[tuple[threading.Thread, bool, str]] = []

    def start_registered_worker(worker: threading.Thread, command_id: str) -> None:
        started_threads.append((worker, worker.is_alive(), command_id))
        worker.start()

    provider.bind_wp8_background_worker_starter(start_registered_worker)
    provider.wp8_move_z_pseudo_home = lambda *_args, **_kwargs: (
        release.wait(1.0) and {"ok": True, "delivery_attempted": True}
    )

    started = provider.wp8_start_move_z_pseudo_home(
        "startMoveZPseudoHome", {}, command_id="registered-command",
        child_order=1, plan_digest="b" * 64,
    )
    worker = provider._wp8_tasks[started["background_task_id"]]["thread"]

    assert started_threads == [(worker, False, "registered-command")]
    assert worker.is_alive() is True
    release.set()
    worker.join(timeout=1.0)


def test_unawaited_wp8_worker_rechecks_fence_before_physical_delivery() -> None:
    provider = _bare_wp8_provider()
    delivered: list[str] = []
    provider.bind_wp8_execution_fence_checker(
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            RuntimeError("deck_execution_interrupted")
        )
    )
    provider.wp8_move_z_pseudo_home = lambda *_args, **_kwargs: (
        delivered.append("controller") or {"ok": True, "delivery_attempted": True}
    )

    started = provider.wp8_start_move_z_pseudo_home(
        "startMoveZPseudoHome", {}, command_id="fenced-background-command",
        child_order=1, plan_digest="c" * 64,
    )
    ...
    assert row["error"]["message"] == "deck_execution_interrupted"
...
def test_production_wp8_snapshot_carries_authoritative_position_table(monkeypatch) -> None:
    ...
    provider.mov_execution_machine_state = lambda: {
        "plate_locations": {0: 23, 1: 21, 3: 29}, "plate_on_gantry": None,
        "tip_loaded": False, "thermal_door_open": None, "gripper_version": 1,
        "board_test_mode": False,
    ...
    assert snapshot["plate_locations"] == {0: 23, 1: 21, 3: 29}
    assert "locations" not in snapshot
    assert snapshot["output_plate_location"] == 21
    assert snapshot["thermal_door_open"] is None
    assert snapshot["door_is_open"] is None
...
def test_wp8_rechecks_interrupt_immediately_before_parent_finalization() -> None:
    class Store:
        def __init__(self):
            self.finalized = None
        def assert_deck_execution_current(self, _command_id, *, boundary=None):
            if boundary == "before_finalize_wp8_operation":
                raise RuntimeError("deck_execution_interrupted")
        def persist_wp8_plan(self, *_args, **_kwargs):
            return None
        def record_delivery_attempt(self, *_args, **_kwargs):
        ...
        def persist_wp8_state_mutation(self, *_args, **_kwargs):
            return None
        def complete_wp8_child(self, *_args, **_kwargs):
            return None
        def assert_wp8_background_tasks_settled(self, *_args, **_kwargs):
            return None
    ...
    plan = oem_deck_movement.compile_finite_plate_operation(
        "park_gantry", source_leaf_available=True,
        current_x=100, current_z=100, park_x=0, park_z=0,
    )
    store = Store()
    with pytest.raises(RuntimeError, match="deck_execution_interrupted"):
        oem_deck_movement.make_wp8_operation_executor(
