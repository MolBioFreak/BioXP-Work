...
def test_v6_migration_holds_exclusive_lifecycle_lock_before_backup(
    tmp_path, monkeypatch,
) -> None:
    original_migration = runtime_store_module._migrate_oem_deck_schema_v6
    monkeypatch.setattr(
        runtime_store_module,
        "_migrate_oem_deck_schema_v6",
    ...
    def inspect_backup_lock(_connection, root, **_kwargs):
        with pytest.raises(TimeoutError, match="lifecycle lock deadline exceeded"):
            with runtime_store_module.runtime_lifecycle_lock(
                root,
                exclusive=False,
                deadline=time.monotonic() + 0.05,
            ):
                pass
        contender = sqlite3.connect(root / "bioxp_runtime.db", timeout=0.05, isolation_level=None)
        try:
            with pytest.raises(sqlite3.OperationalError, match="locked"):
                contender.execute("BEGIN IMMEDIATE")
        finally:
            contender.close()
        raise RuntimeError("exclusive-lock-observed")

    monkeypatch.setattr(runtime_store_module, "_verified_sqlite_backup", inspect_backup_lock)
    with pytest.raises(RuntimeError, match="exclusive-lock-observed"):
        OEMRuntimeStore(tmp_path)
        ...
        with store._authority_write():
            with pytest.raises(sqlite3.IntegrityError):
                store.connection.execute(
                    ...
                    "SET terminal_state='completed',terminal_evidence_json=NULL "
                    "WHERE command_id=? AND child_order=0",
                    (admitted["command_id"],),
                )
    finally:
        store.connection.close()
        ...
        with pytest.raises(sqlite3.DatabaseError):
            with store._transaction() as conn:
                conn.execute(
                    ...
                    (
                        admitted["command_id"], 1, "forged-terminal-insert",
                        "startMoveZPseudoHome", plan["plan_digest"], dispatch_attempt_id,
                        child_one_marker["attempt_sequence"],
                        json.dumps(stamps, sort_keys=True, separators=(",", ":")),
                        json.dumps({"result": {"ok": True}}, sort_keys=True, separators=(",", ":")),
                    ),
                )
