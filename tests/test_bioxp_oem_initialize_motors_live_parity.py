...
class _ZPrimitiveSpy:
    ...
    def z_move_steps(self, steps: int, *, timeout_s: float):
        self.calls.append(("move_steps", {"steps": steps, "timeout_s": timeout_s}))
        return {
            "ok": True,
            "controller_command_acknowledged": True,
            "controller_terminal_state_verified": True,
...
def test_provider_owns_z_lifecycle_and_routes_only_source_positive_intents():
    spy = _ZPrimitiveSpy()
    provider = Serial206OemInitializationProvider(spy, generation_provider=lambda: 1)
