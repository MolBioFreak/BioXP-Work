...
CREATE TRIGGER IF NOT EXISTS operator_plane_wp8_background_tasks_authorized_insert BEFORE INSERT ON operator_plane_wp8_background_tasks
 WHEN authority_write_allowed()<>1
   OR NEW.state<>'created'
   OR NEW.authority_stamps_json<>canonical_json(NEW.authority_stamps_json)
   OR NEW.evidence_json<>'{}'
 BEGIN SELECT RAISE(ABORT,'wp8 background task writer is not authoritative'); END;
