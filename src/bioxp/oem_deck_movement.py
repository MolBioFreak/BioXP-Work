...
def execute_mov_execution(command_id: str, plan: MovExecutionPlan, *, provider: Any, command_store: Any) -> dict[str, Any]:
    ...
    with lease:
        ...
        if plan.machine_state_updates:
            ...
            try:
                if callable(assert_current):
                    assert_current(command_id, boundary="before_initial_semantic_publish")
        ...
        for step in plan.steps:
            ...
            if not callable(method) and not special:
                if callable(terminalize):
                    if callable(assert_current):
                        assert_current(command_id, boundary=f"before_terminalize:{step.order}:{step.operation}")
                ...
                if step.operation == "troughOscillation":
                    ...
                    for child_order, (operation, value) in enumerate(step.arguments["actions"]):
                        if callable(assert_current):
                            assert_current(
                                command_id,
                                boundary=f"before_nested_child:{step.order}:{child_order}:{operation}",
                            )
                        ...
                        if callable(assert_current):
                            assert_current(
                                command_id,
                                boundary=f"after_nested_child:{step.order}:{child_order}:{operation}",
                            )
            ...
            if step.semantic_transition is not None and callable(complete_stage):
                try:
                    if callable(assert_current):
                        assert_current(command_id, boundary=f"before_complete_stage:{step.order}:{step.operation}")
                ...
                if step.semantic_transition is not None:
                    try:
                        if callable(assert_current):
                            assert_current(command_id, boundary=f"before_semantic_publish:{step.order}:{step.operation}")
    ...
    *, provider_getter: Callable[[], Any], command_store: Any,
    ...
    def execute(*, command_id: str, plan: Mapping[str, Any]) -> dict[str, Any]:
        ...
        with lease:
            ...
            fence_binder = getattr(provider, "bind_wp8_execution_fence_checker", None)
            if callable(fence_binder):
                fence_binder(command_store.assert_deck_execution_current)
            worker_binder = getattr(provider, "bind_wp8_background_worker_starter", None)
            if callable(worker_binder):
                worker_binder(command_store.start_wp8_background_worker)
            ...
            def invoke(child: Mapping[str, Any]) -> Any:
                ...
                try:
                    ...
                    command_store.assert_deck_execution_current(
                        command_id, boundary=f"before_terminalize_wp8_child_{order}",
                    )
                ...
                if not bool(child.get("awaited", True)):
                    ...
                    command_store.assert_deck_execution_current(
                        command_id, boundary=f"before_mark_wp8_background_task_{order}",
                    )
                    ...
                    if callable(settle):
                        command_store.assert_deck_execution_current(
                            command_id, boundary=f"before_settle_wp8_background_task_{order}",
                        )
                        settle(
                ...
                if callable(complete_child):
                    command_store.assert_deck_execution_current(
                        command_id, boundary=f"before_complete_wp8_child_{order}",
                    )
                    complete_child(
                    ...
                    if mutation:
                        command_store.assert_deck_execution_current(
                            command_id, boundary=f"before_wp8_state_mutation_{order}",
                        )
            ...
            try:
                ...
                if callable(finalize):
                    command_store.assert_deck_execution_current(
                        command_id, boundary="before_finalize_wp8_operation",
                    )
        ...
        *,
        ...
        with lease:
            ...
            if not callable(persist_plan):
                raise RuntimeError("named_deck_plan_store_not_bound")
            ...
            assert_current(command_id, boundary="before_force_to_high_home")
            try:
                record_delivery(
                    command_id,
                    ...
                    plan_digest=plan.plan_digest,
                    authority_stamps=dispatch_stamps,
                )
            except Exception as exc:
                raise DeckExecutionFailure(
                    f"named_delivery_marker_failed:ForceToHighHome:{type(exc).__name__}",
                    delivery_attempted=False,
                ) from exc
            force_result = force(command_id=command_id)
            try:
                assert_current(command_id, boundary="after_force_to_high_home")
            except Exception as exc:
                raise DeckExecutionFailure(
                    str(exc),
                    delivery_attempted=True,
                    ...
                    controller_completion_verified=bool(
                        isinstance(force_result, Mapping)
                        and force_result.get("controller_completion_verified") is True
                    ),
                    hardware_postcondition_verified=bool(
                        isinstance(force_result, Mapping)
                        and force_result.get("hardware_postcondition_verified") is True
                    ),
                    provider_results=[dict(force_result)]
                    if isinstance(force_result, Mapping)
                    else [],
                ) from exc
            if not isinstance(force_result, Mapping) or force_result.get("ok") is not True:
                if callable(terminalize_stage):
                    terminalize_stage(
                        ...
                        plan.steps[0],
                        state="ambiguous",
                        result=dict(force_result) if isinstance(force_result, Mapping) else None,
                        reason="force_to_high_home_failed_after_delivery",
                    )
                return {
                    "ok": False,
                    "admitted": True,
            ...
            if callable(persist_pseudo):
                stamp_reader = getattr(provider, "deck_owner_authority_stamps", None)
                raw_stamps = stamp_reader() if callable(stamp_reader) else None
                stamps = dict(raw_stamps) if isinstance(raw_stamps, Mapping) else dispatch_stamps
                persist_pseudo(
