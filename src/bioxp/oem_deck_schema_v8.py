"""V8: named delivery binds independent owner epochs, not semantic completeness.

V1-V7 definitions and identities are frozen. No retained operational row is
rewritten. Full/script work retains its existing semantic lineage checks.
"""
...
VERSION = 8
TRIGGER = 'operator_plane_delivery_attempts_lineage_insert'
_sources = [s for s in _statements(DECK_SCHEMA_V6_EXTRA_SQL)
            if s.startswith('CREATE TRIGGER IF NOT EXISTS ' + TRIGGER + ' BEFORE')]
if len(_sources) != 1:
    raise RuntimeError('frozen delivery lineage source is not exact')
OLD = '''     AND semantic.ownership_generation=NEW.ownership_generation
     AND semantic.board_epoch_4=NEW.board_epoch_4 AND semantic.board_epoch_5=NEW.board_epoch_5'''
NEW = '''     AND m.state='dispatched'
     AND (
       (c.action_id='oem.deck.move_to_location' AND NEW.work_kind='named_stage'
        AND deck_owner_authority_current(NEW.ownership_generation,NEW.board_epoch_4,NEW.board_epoch_5)=1)
       OR (c.action_id<>'oem.deck.move_to_location'
           AND semantic.ownership_generation=NEW.ownership_generation
           AND semantic.board_epoch_4=NEW.board_epoch_4 AND semantic.board_epoch_5=NEW.board_epoch_5)
     )'''
if _sources[0].count(OLD) != 1:
    raise RuntimeError('frozen delivery semantic predicate is not exact')
...
def migration_identity():
    ...
    return RuntimeMigrationIdentity(version=VERSION, name='oem_deck_scoped_authority_v8',
        ddl_sha256=hashlib.sha256(inspect.getsource(sys.modules[__name__]).encode()).hexdigest())


def verify(connection):
    """Exact SQL attestation of unchanged v7 plus the one v8 trigger.
    ...
    if set(actual) != set(expected):
        raise RuntimeError('canonical deck schema v8 object manifest is not exact')
    for identity, sql in expected.items():
        ...
        if hashlib.sha256(actual[identity].encode()).hexdigest() not in accepted:
            raise RuntimeError('canonical deck schema v8 SQL is not exact:' + identity[1])
    if connection.execute('PRAGMA foreign_key_check').fetchone() is not None:
        raise RuntimeError('canonical deck schema v8 foreign-key check failed')
...
def migrate(connection, root: Path, identity):
    ...
    with lifecycle:
        ...
        if connection.execute('PRAGMA user_version').fetchone()[0] != 7:
            raise RuntimeError('scoped deck migration requires exact v1-v7 prefix')
        started = time.time()
        connection.execute('BEGIN IMMEDIATE')
        try:
            owner.verify_canonical_runtime_database(connection, version=7, full_data_check=True)
