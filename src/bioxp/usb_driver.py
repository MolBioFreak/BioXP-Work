...
class BioXpTester:
    ...
    # OEM/decompiled ClassMotor/ClassDeckBoard/ClassHeadBoard switch semantics:
    # GAP9/GAP10 raw value 1 means the switch line is asserted.  The C# helper
    # queryLeftSwitchStatus()/queryRightSwitchStatus() converts raw 1 to return
    # code 0, and board-level queryHome()/queryRightSensor() then treat that
    # return-code 0 as True.  The Linux driver exposes the raw GAP value, so the
    # active/home raw predicate must be 1, not the OEM helper return code 0.
    ...
    def __init__(self, alt=1):
        # The Novo USB endpoints are one request/reply stream.  Board and command
        # fields are insufficient to correlate concurrent same-command traffic on
        # different motors, so one owner must retain the endpoints for the complete
        # drain/write/reply transaction and for endpoint replacement.
        ...
        self._bus_event_sequence = 0
        self._bus_event_buffer_max = 256
        self._oem_position_cache = {}
        self._oem_speed_cache = {}
        self._oem_motor_home_cache = {}
        # OEM ClassBaseBoard.m_24Vdropped latch.  This is changed only by the
        ...
        self._motion_latch_override = {
            "enabled": False,
            "override_latch_sensor": False,
            "override_rail_24v": False,
            "reason": "startup",
            "note": "auto-lock mode",
            "updated_ms": int(time.time() * 1000),
            "seq": 0,
    ...
    def _transport_guard(self):
        ...
        A lazy fallback preserves isolated tests and recovered callers that build a
        tester with ``__new__`` instead of executing ``__init__``.
        """
    ...
    def _connect(self):
        with self._transport_guard():
            ...
            if self.ep_out is None or self.ep_in is None:
                raise ValueError("BioXP Novo USB endpoints were not found.")
            if self.novo_router is not None:
                raise RuntimeError("Novo router already exists during USB connect")
            # This USB owner retains the logging worker across cleanup until it
            # actually exits. Reconnect cannot pile up blocked SQLite writers.
            from .receiver_audit_buffer import ReceiverAuditBuffer
            audit = getattr(self, "_receiver_audit", None)
            if audit is None or not audit.alive:
                try:
                    audit = ReceiverAuditBuffer.from_environment()
                    self._receiver_audit = audit
                    self._receiver_audit_setup_error = None
                except Exception as exc:
                    self._receiver_audit_setup_error = repr(exc)[:512]
            self.novo_router = NovoRouter(ep_in=self.ep_in, ep_out=self.ep_out,
    ...
    def _disconnect(self, dev=None, *, hard_reset=False, return_device=False):
        with self._transport_guard():
            ...
            if router is not None:
                ...
                if summary["router_shutdown_ok"] is True:
                    # Audit failure is not USB-reader cleanup failure.
                    audit = getattr(self, "_receiver_audit", None)
                    if audit is not None:
                        try:
                            summary["receiver_audit"] = audit.close(timeout_s=0.0)
                        except Exception as exc:
                            summary["receiver_audit_error"] = repr(exc)[:512]
            if dev is self.dev:
            ...
            try:
                ...
                summary["release_interface_ok"] = True
            except Exception as exc:
    ...
    def _reset_transport_recovery_state(self):
        ...
        self._motor_last_tx_ts = {int(bid): 0.0 for bid in self.MOTOR_BOARDS}
        self._motor_noresp_streak = {int(bid): 0 for bid in self.MOTOR_BOARDS}
        self._oem_board_initialized = {int(board): False for board in self.BOARDS}
        self._oem_transport_generation = int(getattr(self, "_oem_transport_generation", 0)) + 1
        self._invalidate_oem_no_motion_profiles(reason="transport_reconnected")

    def reconnect(self, *, hard_reset=False):
        with self._transport_guard():
            return self._reconnect_locked(hard_reset=hard_reset)

    def _reconnect_locked(self, *, hard_reset=False):
        ...
        for idx, use_hard_reset in enumerate(attempts):
            ...
            try:
                ...
                recovery_dev = self._disconnect(disconnect_target, hard_reset=use_hard_reset, return_device=True)
    ...
    def _decode_bus_event_frame(self, resp, source="usb_in"):
        frame = list(resp)
        if tuple(frame) in self.KNOWN_UNSOLICITED_FRAMES or len(frame) < 14:
            return None
        try:
            normalized = novo_decode(bytes(frame))
            if len(normalized) != 13 or normalized[4] != 8 or int.from_bytes(normalized[:4], "big") not in (0, 99):
                return None
            data = normalized[5:]
            status = int(data[1])
            value = struct.unpack(">i", bytes(data[3:7]))[0]
        except Exception:
            return None
        # ClassHeadBoard uses normalized payload[6] for target axis and [2]
        # for stall axis. USB escape bytes must never become field offsets.
        motor = None
        if status == 128:
            candidate = int(data[6])
            motor = candidate if 0 <= candidate <= 2 else None
        elif status == 130:
            candidate = int(data[2])
            motor = candidate if 0 <= candidate <= 2 else None
        return {
            "board": int(data[0]),
            "status": status,
    ...
    def clear_bus_event_buffer(self):
        ...
        router_cleared = {}
        router = getattr(self, "novo_router", None)
        clear_queue = getattr(router, "queue_clear", None) if router is not None else None
        if callable(clear_queue):
            for name in ("valid_async", "unknown_async"):
    ...
    def begin_bus_event_window(self, *, reset_wait_latch=True):
        ...
        return {
            **cursor,
            ...
            "cleared": 0,
            "router_cleared": {},
            "queue_purge_omitted_by_source": True,
            "preserved_for_oem_null_query_stop": not reset,
        }
    ...
    def drain(self, max_reads=16, timeout_ms=8):
        with self._transport_guard():
            return self._drain_locked(max_reads=max_reads, timeout_ms=timeout_ms)

    def _drain_locked(self, max_reads=16, timeout_ms=8):
        del max_reads, timeout_ms
        router = getattr(self, "novo_router", None)
        if router is None:
            return []
        return [frame.provenance() for frame in router.queue_snapshot("valid_async")]
    ...
    def _send_tmcl_locked(
        ...
        if router is None:
            return None
        try:
            # ClassNovo.TransmitMessage: global sendingLock encloses Sleep(1),
            # the whole sendCommand (including its resend), then Sleep(10).
            # Only source-selected motor calls are reconciled here.
            ...
            # Normal null still gets post10ms; a thrown USB exception does not.
            if ordinary_motor_retry:
                time.sleep(0.010)
    ...
    def collect_bus_events(self, duration_s=0.50, timeout_ms=20, max_events=96):
        ...
        with self._transport_guard():
            return self._collect_bus_events_locked(
                duration_s=duration_s,
                timeout_ms=timeout_ms,
                max_events=max_events,
            )

    def _collect_bus_events_locked(self, duration_s=0.50, timeout_ms=20, max_events=96):
        """Actively retain and decode asynchronous TMCL frames for one window."""
        router = getattr(self, "novo_router", None)
        if router is None:
            return []
        deadline = time.monotonic() + max(0.0, float(duration_s))
        sleep_s = max(0.001, min(0.02, float(timeout_ms) / 1000.0))
        while time.monotonic() < deadline:
            time.sleep(min(sleep_s, max(0.0, deadline - time.monotonic())))
        frames = router.queue_snapshot("valid_async") + router.queue_snapshot("unknown_async")
        decoded = []
        for frame in frames[-int(max_events):]:
            ...
            if event is not None:
                ...
                decoded.append({**provenance, "status": None, "source": "novo_router_async_unclassified"})
        return decoded
    ...
    def _record_oem_board_activation(self, board_id, *, active, ack):
        ...
        if active:
            state[int(board_id)] = True
        else:
            state[int(board_id)] = False
        home_cache = getattr(self, "_oem_motor_home_cache", None)
        if not isinstance(home_cache, dict):
            home_cache = {}
            self._oem_motor_home_cache = home_cache
        for key in tuple(home_cache):
            if isinstance(key, tuple) and len(key) == 2 and int(key[0]) == int(board_id):
                home_cache[key] = False
    ...
    def _invalidate_oem_no_motion_profiles(self, *, reason):
        """Invalidate every axis profile derived from the previous board state."""
        self._oem_no_motion_profile_ready = False
        self._oem_no_motion_profiles_ready = set()
        self._oem_no_motion_profile_generations = {}
        self._oem_no_motion_profile_fingerprints = {}
        self._oem_active_board_lifecycle_generation = None
        self._oem_no_motion_profile_invalidation = {
            "reason": str(reason),
    ...
    def _set_boards_active(self, active: bool, expect_reply=True, fail_fast=False):
        ...
        # Any command-64 transition invalidates every source motor profile until
        # initializeMotorsWithoutMotion establishes it again.
        self._invalidate_oem_no_motion_profiles(
            reason="cmd64_activate" if bool(active) else "cmd64_deactivate"
        )
        out = {}
        ...
        for bid in self.BOARDS:
            ack = self.send_tmcl_retry(
                ...
                max_reads=12,
                strict_match=True,
                # OEM ClassChillerBoard.activateBoard() checks the returned
    ...
    def oem_begin_board_lifecycle_generation(self, *, deactivation, activation):
        """Mint a generation only after a complete OEM cmd64=0 -> cmd64=1 cycle."""
        deactivate_ok = self._oem_board_activation_map_success(deactivation)
        activate_ok = self._oem_board_activation_map_success(activation)
        if not deactivate_ok or not activate_ok:
            self._oem_active_board_lifecycle_generation = None
            return {
                "ok": False,
                "failure": "incomplete_oem_deactivate_activate_cycle",
                "deactivation_complete": bool(deactivate_ok),
                "activation_complete": bool(activate_ok),
            }
        ...
        return {
            ...
            "transport_generation": int(getattr(self, "_oem_transport_generation", 0)),
    ...
    def _oem_board_present(self, board_id):
        ...
        configured = getattr(self, "_oem_board_presence", None)
        if isinstance(configured, dict):
            return bool(configured.get(int(board_id), False))
        return int(board_id) in self.BOARDS

    def motor_oem_axis_board_present(self, axis: str) -> bool:
        profile = self._motion_oem_axis_profile(str(axis).strip().lower(), startup=True)
        return bool(isinstance(profile, Mapping) and self._oem_board_present(int(profile["board"])))

    def motor_oem_axis_board_initialized(self, axis: str) -> bool:
        profile = self._motion_oem_axis_profile(str(axis).strip().lower(), startup=True)
        return bool(isinstance(profile, Mapping) and self._oem_board_present(int(profile["board"])) and self._oem_board_state().get(int(profile["board"]), False))
    ...
    def _oem_activate_board(self, board_id):
        ...
        ack = self.send_tmcl_retry(
            ...
            require_command_echo=bid != self.BOARD_CHILLER,
        )
    ...
    def motor_oem_wait_for_board(
        ...
        *,
        ...
        """Execute ClassControlInterface.waitForBoard source control flow.

        OEM waits indefinitely while a board remains uninitialized.  The
        optional timeout is retained only as an explicit host-side containment
        argument; the default path is the OEM unbounded wait.
        """
        ...
        while True:
            state = self._oem_board_state()
            required = [int(board) for board in self.BOARDS if self._oem_board_present(board)]
            initialized = {board for board in required if bool(state.get(board, False))}
            missing = [board for board in required if board not in initialized]
            ...
            remaining = None if timeout_s is None else max(0.0, timeout_s - elapsed)
            sleep(poll_interval_s if remaining is None else min(poll_interval_s, remaining))
            polls += 1
            ...
            if not missing:
                return {
                    ...
                    "activations": activations,
                    "source_anchor": "ClassControlInterface.waitForBoard lines 3507-3534",
                    "linux_guardrail": None if timeout_s is None else "bounded_wait_for_board",
                }
            if activation_counter >= activation_every_polls:
                activation_counter = 0
                # OEM waitForBoard calls ControlInterface.activateBoard(),
                # which activates only boards whose m_isinitialized flag is
                # still false.  It does not reactivate every board.
                activation = self.oem_activate_uninitialized_boards()
                activations.append(activation)


    def oem_wait_for_board(self, **kwargs):
        """Compatibility name for the bounded Linux OEM wait contract."""
        return self.motor_oem_wait_for_board(**kwargs)
    ...
    def set_board_activation_observer(self, observer):
        self._board_activation_observer = observer

    def board_activate(self, board_id):
        self._invalidate_oem_no_motion_profiles(reason="cmd64_activate_single_board")
        ack = self.send_tmcl_retry(
            int(board_id),
            64,
            0,
    ...
    def board_deactivate(self, board_id):
        self._invalidate_oem_no_motion_profiles(reason="cmd64_deactivate_single_board")
        ack = self.send_tmcl_retry(
            int(board_id),
            64,
            0,
    ...
    def query_only_tmcl(self, board_id, command, cmd_type, motor=0, value=0):
        """Issue one read/query transaction with recovery and activation disabled.

        This is the sole low-level boundary used by the canonical POST collector.
        Callers must supply a query command; this method never activates a board,
        changes power/configuration, drains traffic, reconnects, or retries through
        a recovery path.
        """
        board_id = int(board_id)
        kwargs = {
        ...
        if board_id == int(self.BOARD_THERMAL):
    ...
    def query_only_transport_state(self):
        ...
        router = getattr(self, "novo_router", None)
        can_ready = None if router is None else bool(getattr(router, "running", False))
        return {
            "CAN_READY": can_ready,
            "usb_bound": getattr(self, "dev", None) is not None,
            "router_running": can_ready,
            "allow_recover": False,
        }
    ...
    def _send_motor(
        ...
        *,
        ...
        try:
            ...
            # Final guard: if a USBError slipped past send_tmcl catches
            # (e.g. from drain()), treat it as a no-response so the
            # streak counter triggers reconnect().
            ack = None
        if wait_reply:
    ...
    def motor_set_axis_param(self, board_id, param, value, motor=0):
        ...
        return {
            ...
            "controller_command_acknowledged": ack_success,
    ...
    def motor_get_position(self, board_id, motor=0):
        ...
        cache = getattr(self, "_oem_position_cache", None)
        if not isinstance(cache, dict):
            cache = {}
            self._oem_position_cache = cache
        reply_valid = bool(self._tmcl_success(row.get("ack")) and type(row.get("value")) is int)
        if reply_valid:
            ...
            position = 1
            position_source = "oem_error_sentinel"
        return {
            "board": int(board_id),
            "motor": int(motor),
            "ack": row.get("ack"),
            "position": position,
            "ok": reply_valid,
            "position_reply_valid": reply_valid,
    ...
    def motor_get_speed(self, board_id, motor=0):
        ...
        key = (int(board_id), int(motor))
        cache = getattr(self, "_oem_speed_cache", None)
        if not isinstance(cache, dict):
            cache = {}
            self._oem_speed_cache = cache
        first_ack = row.get("ack")
        reply_valid = bool(self._tmcl_success(first_ack) and type(row.get("value")) is int)
        if first_ack is None:
            # ClassMotor.queryMotorSpeed retries once only when the first
            # getParameter call returns null. It does not inspect the retry;
            # the method immediately returns the prior m_currentSpeed cache.
            self.motor_get_axis_param(board_id, 3, motor=motor)
            speed = int(cache.get(key, 0))
            speed_source = "oem_cached"
            reply_valid = False
        elif reply_valid:
            speed = int(row["value"])
            cache[key] = speed
            speed_source = "controller_reply"
        else:
            # A non-null controller error returns the literal OEM sentinel.
            speed = 100000
            speed_source = "oem_error_sentinel"
        return {
            "board": int(board_id),
            "motor": int(motor),
            "ack": first_ack,
    ...
    def motor_oem_set_xy_current_mode(self, enabled, *, sleep_fn=time.sleep):
        ...
        def write(axis, board, motor, value, stage):
            ...
            acknowledged = bool(isinstance(row, dict) and self._tmcl_success(row.get("ack")))
            writes.append({
                "axis": axis,
                "board": board,
                "motor": motor,
        ...
        if enabled:
            for axis, board, motor in present:
                write(axis, board, motor, 10, "warm_10")
            sleep_fn(0.500)
            for axis, board, motor in present:
                write(axis, board, motor, 31, "run_31")
        else:
            for axis, board, motor in present:
                write(axis, board, motor, 1, "disabled_1")
        acknowledged = bool(writes) and all(
            row["controller_command_acknowledged"] for row in writes
        )
        return {
            "ok": True,
            "intent": "enableXY",
            ...
            "failure": None,
            "controller_evidence_failure": None if acknowledged else "enableXY_current_write_not_acknowledged",
        }
    ...
    def motor_oem_set_xyz_current_mode(self, enabled, *, z_current_up=31, sleep_fn=time.sleep):
        ...
        effective_z_current = max(0, min(31, int(z_current_up)))
        writes = []
        ...
        if enabled:
            ...
            sleep_fn(0.500)
            if z_present:
                write("z", int(self.BOARD_HEAD), 1, effective_z_current, "z_current_up")
        else:
            for axis, board, motor in present_xy:
                write(axis, board, motor, 1, "disabled_1")
            if z_present:
                write("z", int(self.BOARD_HEAD), 1, 1, "disabled_1")
        z_current_readback = (
            self.motor_get_axis_param(int(self.BOARD_HEAD), 6, motor=1)
            if z_present
            else None
        )
        acknowledged = bool(writes) and all(
        ...
        z_read_acknowledged = bool(
            not z_present
            or (
                isinstance(z_current_readback, dict)
                and self._tmcl_success(z_current_readback.get("ack"))
            )
        )
        terminal_verified = bool(acknowledged and z_read_acknowledged)
        return {
            "ok": True,
            "intent": "enableXYZ",
            "enabled": enabled,
            ...
            "z_current_up_requested": int(z_current_up),
            "z_current_up": effective_z_current,
            "z_current_readback": z_current_readback,
            "gripper_current_written": False,
            "source_call_completed": True,
            "physical_motion_commanded": False,
            "controller_command_acknowledged": acknowledged,
            ...
            "failure": None,
            "controller_evidence_failure": None if terminal_verified else "enableXYZ_current_transaction_not_acknowledged",
        }
    ...
    def motor_get_reached_position(self, board_id, motor=0):
        ...
        # parse the returned value as movement proof. Normal move completion is
        # the async TARGET_POSITION_REACHED(128) event that sets _waitForMotor.
        row = self.motor_get_axis_param(board_id, 8, motor=motor)
        ack = row.get("ack") if isinstance(row, dict) else None
        ack_success = self._tmcl_success(ack)
        source_return = 0 if ack is None or ack_success else 1
        return {
            "board": int(board_id),
            "motor": int(motor),
            "gap_param": 8,
            ...
            "ack_success": bool(ack_success),
            "oem_query_reached_position_return": source_return,
            "target_reached": None,
            "ok": bool(ack_success),
        }

    def motor_get_switches(self, board_id, motor=0):
        # GAP param 9/10 = raw left/right switch status in the OEM path.
        # GAP param 12/13 = controller-side right/left switch-disable masks.
        # Important: masked switch lines can read as the active raw value.  Report
        # raw state separately from effective (unmasked) activity so a disabled
        # right switch is not misreported as a physical limit hit while mid-deck.
        left = self.motor_get_axis_param(board_id, 9, motor=motor)
        right = self.motor_get_axis_param(board_id, 10, motor=motor)
        right_disable = self.motor_get_axis_param(board_id, 12, motor=motor)
        left_disable = self.motor_get_axis_param(board_id, 13, motor=motor)
        left_state = left.get("value")
        right_state = right.get("value")
        left_disabled = bool(int(left_disable.get("value") or 0) != 0)
        right_disabled = bool(int(right_disable.get("value") or 0) != 0)
        active_val = int(self.MOTOR_SWITCH_ACTIVE_VALUE)
        left_raw_active = None if left_state is None else (int(left_state) == active_val)
        right_raw_active = None if right_state is None else (int(right_state) == active_val)
        return {
            "board": int(board_id),
            "motor": int(motor),
            "left": left,
            "right": right,
            "left_disable": left_disable,
            "right_disable": right_disable,
            ...
            "left_effective_active": None if left_raw_active is None else (bool(left_raw_active) and not left_disabled),
            "right_effective_active": None if right_raw_active is None else (bool(right_raw_active) and not right_disabled),
        }

    def motor_get_switch_activity(self, board_id, motor=0):
        """
        OEM/decompiled switch semantics projected onto raw Linux GAP reads:
        - raw left/home assertion when GAP9 == MOTOR_SWITCH_ACTIVE_VALUE (1)
        - raw right/open assertion when GAP10 == MOTOR_SWITCH_ACTIVE_VALUE (1)
        - effective physical activity excludes controller-side SAP12/SAP13 masks.
        """
        sw = self.motor_get_switches(board_id, motor=motor)
        left = sw.get("left_state")
        right = sw.get("right_state")
        active_val = int(self.MOTOR_SWITCH_ACTIVE_VALUE)
        left_raw_active = None if left is None else (int(left) == active_val)
        right_raw_active = None if right is None else (int(right) == active_val)
        left_disabled = bool(sw.get("left_disabled"))
        right_disabled = bool(sw.get("right_disabled"))
        return {
            "board": int(board_id),
            "motor": int(motor),
    ...
    def motor_query_24v_sensor(self):
        ...
        status = None if ack is None else ack.get("status")
        payload_raw = None if ack is None or status != 100 else ack.get("value")
        reply_valid = bool(ack is not None and status == 100 and payload_raw is not None)
        if ack is None:
            oem_scalar = 0
            ...
            oem_scalar = int(payload_raw)
            scalar_source = "OEM_status_100_payload_byte_6"
        else:
            oem_scalar = None
            scalar_source = "malformed_status_100_missing_payload"
        oem_no24v = None if oem_scalar is None else bool(oem_scalar != 0)
        if oem_no24v is not None:
            # Exact ClassDeckBoard.query24VSensor latch transition.
            self._oem_24v_dropped = bool(oem_no24v)
            if not oem_no24v:
                self._oem_user_stopped = False
        return {
            "ack": ack,
            "payload_raw": payload_raw,
            "oem_scalar": oem_scalar,
            "oem_scalar_source": scalar_source,
            "oem_no24v": oem_no24v,
            "sample_valid": reply_valid,
            "reply_valid": reply_valid,
            "reply_present": ack is not None,
            "transport_outcome": "reply" if ack is not None else "no_reply",
            "oem_status": status,
            "no24v": oem_no24v,
            "safety_valid": bool(reply_valid and oem_no24v is False),
            "zero_valid_sample": bool(reply_valid and oem_scalar == 0),
            "no_valid_sample": not reply_valid,
        }

    def oem_no24v_state(self):
        """Return the retained OEM ClassBaseBoard.No24V latch."""
        return bool(getattr(self, "_oem_24v_dropped", False))

    def oem_latch_24v_dropped(self, *, reason="forceAbortMotion"):
        """Compatibility alias for the exact board-aware OEM abort transition."""
        return self.motor_oem_force_abort_motion(reason=reason)

    def motor_oem_force_abort_motion(self, *, reason="forceAbortMotion"):
        """Apply ClassControlInterface.forceAbortMotion board-level state changes."""
        self._oem_24v_dropped = True
        self._oem_abort_generation = int(getattr(self, "_oem_abort_generation", 0)) + 1
        time.sleep(0.001)
        affected = []
        if self._oem_board_present(self.BOARD_HEAD):
            self._oem_24v_dropped = True
            affected.extend(("y", "z", "g"))
            self._oem_set_abort_waiters(self.BOARD_HEAD, (0, 1, 2))
        if self._oem_board_present(self.BOARD_DECK):
            self._oem_24v_dropped = True
            affected.append("x")
            self._oem_set_abort_waiters(self.BOARD_DECK, (0,))
        if self._oem_board_present(self.BOARD_THERMAL):
            affected.append("door")
            # ClassThermalBoard.forceAbortMotion disables the board's internal
            # thermal-control timer. It does not stop ClassThermalFanControl's
            # independent five-second fan service.
            self._oem_thermal_board_timer_enabled = False
            self._oem_set_abort_waiters(self.BOARD_THERMAL, (0,))
            self._oem_user_stopped = True
            self._oem_24v_dropped = True
        return {
            "ok": True,
            "no24v": True,
            "latched": True,
            "reason": str(reason),
            "abort_generation": self._oem_abort_generation,
            "affected_waiters": affected,
            "source_anchor": "ClassControlInterface.forceAbortMotion; board forceAbort overrides",
        }

    def _oem_set_abort_waiters(self, board, motors):
        setter = getattr(getattr(self, "novo_router", None), "set_motor_abort_event", None)
        for motor in motors:
            if callable(setter):
                setter(board, motor)
            else:
                with self._transport_guard():
                    signals = getattr(self, "_oem_abort_signals", None)
                    if signals is None:
                        signals = self._oem_abort_signals = set()
                    signals.add((int(board), int(motor)))
    ...
    def motion_latch_override_state(self):
        if isinstance(self._motion_latch_override, dict):
            ...
            state = {"enabled": False, "reason": "unset", "note": None, "updated_ms": None, "seq": 0}
        state.setdefault("override_latch_sensor", bool(state.get("enabled", False)))
        state.setdefault("override_rail_24v", False)
        state["enabled"] = bool(state.get("override_latch_sensor") or state.get("override_rail_24v"))
        state.setdefault("reason", "unset")
        state.setdefault("note", None)
        state.setdefault("updated_ms", None)
        state.setdefault("seq", 0)
        return state

    def motion_latch_override_set(self, enabled=None, reason="manual", *, override_latch_sensor=None, override_rail_24v=None):
        """
        Toggle commissioning-only software bypasses for faulty/misaligned latch and 24V sense inputs.

        This does not move axes and it still commands the physical latch lock before enabling the bypass.
        The override only changes software gate interpretation:
        - override_latch_sensor=True treats the latch reed/mag sensor as permissive.
        - override_rail_24v=True treats the 24V sense line as permissive.
        Door-closed and solenoid-lock readbacks remain real checks.
        """
        prev = self.motion_latch_override_state()
        prev_seq = int(prev.get("seq", 0) or 0)
        if override_latch_sensor is None:
            override_latch_sensor = bool(enabled) if enabled is not None else bool(prev.get("override_latch_sensor"))
        if override_rail_24v is None:
            override_rail_24v = bool(enabled) if enabled is not None else bool(prev.get("override_rail_24v"))
        target_latch = bool(override_latch_sensor)
        target_rail = bool(override_rail_24v)
        target_enabled = bool(target_latch or target_rail)

        attempts = []
        # Keep the physical latch lock asserted in both default and override modes.
        attempts.append({"op": "latch_oem_lock", "write": self.latch_oem(True)})
        snap = self.io_snapshot(self.BOARD_DECK)
        sol = self._safe_int(snap.get(2)) if isinstance(snap, dict) else None
        if sol != 1:
            ...
            sol = self._safe_int(snap.get(2)) if isinstance(snap, dict) else None
        verified_lock = sol == 1
        wr = attempts[-1].get("write") if attempts else None
        if target_enabled:
            note = "commissioning_interlock_override_latch_locked" if verified_lock else "commissioning_interlock_override_lock_unverified"
        else:
            note = "auto_lock" if verified_lock else "auto_lock_unverified"

        self._motion_latch_override = {
            "enabled": bool(target_enabled),
            "override_latch_sensor": bool(target_latch),
            "override_rail_24v": bool(target_rail),
            "reason": str(reason),
            "note": note,
            "updated_ms": int(time.time() * 1000),
            "seq": prev_seq + 1,
        ...
        return {
            ...
            "write": wr,
            "attempts": attempts,
            "verified": bool(verified_lock),
            "requested_enabled": bool(target_enabled),
            "requested_override_latch_sensor": bool(target_latch),
            "requested_override_rail_24v": bool(target_rail),
            "gate": live,
    ...
    def motion_gate_live_snapshot(self):
        ...
        ov_latch = bool(ov.get("override_latch_sensor", ov.get("enabled", False)))
        ov_rail = bool(ov.get("override_rail_24v", False))
        io_ok = (door is not None) and (solenoid is not None) and (latch is not None)
        raw_rail_ok = rail.get("no24v") is False
        checks = {
            "rail_ok": True if ov_rail else raw_rail_ok,
            "rail_sensor_raw_ok": raw_rail_ok,
            "rail_24v_override_active": ov_rail,
            "io_read_ok": io_ok,
        }
        error_keys = [k for k, v in checks.items() if k in ("rail_ok", "io_read_ok") and v is not True]
        if not io_ok:
            ...
            checks.update(
                {
                    "door_closed": door == 1,
                    "latch_closed": True if ov_latch else (latch == 1),
                    "latch_sensor_raw_ok": latch == 1,
                    "latch_sensor_override_active": ov_latch,
    ...
    def motion_arm_strict_startup(self, run_homing=True):
        ...
        inter_override = self.motion_latch_override_state()
        inter_rail_raw_ok = inter_rail.get("no24v") is False
        inter_rail_override_active = bool(inter_override.get("override_rail_24v", False))
        inter_rail_ok = bool(inter_rail_raw_ok or inter_rail_override_active)
        add_check(
            "interlock_24v",
            inter_rail_ok,
            detail={
                "raw": inter_rail.get("raw"),
                "no24v": inter_rail.get("no24v"),
                "raw_ok": bool(inter_rail_raw_ok),
                "override_rail_24v": bool(inter_rail_override_active),
                "override_seq": inter_override.get("seq"),
            },
        )
        latch_ack = None
        ...
        if bool(run_homing):
            raise RuntimeError(
                "legacy startup homing is retired; use Serial206OemInitializationProvider.initialize_motors"
            )
    ...
    def _take_fallback_motor_signals(self, targets, event_window):
        """Consume all requested fallback AutoReset handles or none."""
        with self._transport_guard():
            initial = self._oem_initial_motor_signals()
            aborted = getattr(self, "_oem_abort_signals", set())
            wires = getattr(self, "_oem_fallback_motor_signals", {})
            generation = int(getattr(self, "_oem_abort_generation", 0))
            after = event_window.get("after_sequence") if isinstance(event_window, dict) else None
            available = {}
            for key in targets:
                if key in aborted:
                    available[key] = {"board": key[0], "motor": key[1],
                                      "source": "board.forceAbortMotion", "physical_effect_verified": False}
                elif key in initial:
                    available[key] = {"board": key[0], "motor": key[1],
                                      "source": "ClassMotor.initialState", "physical_effect_verified": False}
                elif key in wires:
                    stamp, event = wires[key]
                    sequence = event.get("event_sequence")
                    if stamp == generation and (after is None or
                            (sequence is not None and int(sequence) > int(after))):
                        available[key] = event
            ...
            for key in targets:
                ...
                aborted.discard(key)
                wires.pop(key, None)
            return {f"{board}:{motor}": event for (board, motor), event in available.items()}

    def _collect_fallback_motor_signals(self, duration_s):
        # Use the existing sole endpoint lock for receive, Set, Reset and take.
        # Do not hold it across the complete timeout: Abort must be able to Set.
        with self._transport_guard():
            generation = int(getattr(self, "_oem_abort_generation", 0))
            window = self.collect_bus_events(duration_s=duration_s, timeout_ms=8, max_events=32)
            if generation != int(getattr(self, "_oem_abort_generation", 0)):
                return window  # Do not restamp pre-Abort wire events into its generation.
            wires = getattr(self, "_oem_fallback_motor_signals", None)
            if wires is None:
                wires = self._oem_fallback_motor_signals = {}
            generation = int(getattr(self, "_oem_abort_generation", 0))
            for event in window:
                if not isinstance(event, dict) or event.get("status") != 128:
                    continue
                board, motor = event.get("board"), event.get("motor")
                if type(board) is not int or type(motor) is not int:
                    continue
                if (board, motor) not in {(4, 0), (4, 1), (4, 2), (5, 0), (6, 0)}:
                    continue
                # Set on an already-signaled handle coalesces; preserve latest
                # provenance so an explicit sequence reset rejects older input.
                wires[(board, motor)] = (generation, event)
            return window
    ...
    def motor_query_motor_stop(self, board_id, motor=0):
        ...
        if callable(reset):
            ...
            with self._transport_guard():
                channel = (int(board_id), int(motor))
                self._oem_initial_motor_signals().discard(channel)
                getattr(self, "_oem_abort_signals", set()).discard(channel)
                getattr(self, "_oem_fallback_motor_signals", {}).pop(channel, None)
        return {
            "board": int(board_id),
            "motor": int(motor),
            "ack": ack,
            ...
            if frames or not targets:
                for frame in frames:
                    if isinstance(frame, dict):
                        reached[f"{frame['board']}:{frame['motor']}"] = frame
                        continue
    ...
    def motor_oem_wait_target_reached(
        ...
        *,
        ...
        if callable(getattr(getattr(self, "novo_router", None), "take_motor_events", None)):
            ...
            aborted = bool(signal and signal.get("source") == "board.forceAbortMotion")
            event = None if initial or aborted else signal
            result.update(board=int(board_id), motor=int(motor), event=event, target_reached=bool(result["ok"] and event and event.get("status") == 128),
                          source_wait_signaled=result["ok"],
                          completion_class="oem_abort_latch" if aborted else "oem_initial_latch" if initial else "event_128" if event else None,
                          physical_effect_verified=False)
        ...
        started = time.monotonic()
        deadline = started + max(0.0, float(timeout_s))
        targets = {(int(board_id), int(motor))}
        events = []
        signals = self._take_fallback_motor_signals(targets, event_window)
        first_poll = True
        while signals is None and (first_poll or time.monotonic() <= deadline):
            first_poll = False
            events.extend(self._collect_fallback_motor_signals(
                min(0.020, max(0.0, deadline - time.monotonic()))))
            signals = self._take_fallback_motor_signals(targets, event_window)
        signal = (signals or {}).get(f"{int(board_id)}:{int(motor)}")
        event = signal if signal and signal.get("status") == 128 else None
        source = signal.get("source") if signal else None
        return {"ok": signals is not None, "source_wait_signaled": signals is not None,
                "target_reached": event is not None, "board": int(board_id), "motor": int(motor),
                "event": event, "events": events, "reached": signals or {},
                "completion_class": "event_128" if event else
                    "oem_abort_latch" if source == "board.forceAbortMotion" else
                    "oem_initial_latch" if signal else None,
                "physical_effect_verified": False, "event_window": event_window,
                "failure": None if signals is not None else "oem_moveToAbs_target_event_timeout",
                "elapsed_ms": int((time.monotonic() - started) * 1000)}

    def motor_wait_target_reached(self, board_id, motor=0, timeout_s=20.0, *, event_window=None):
    ...
    def motor_wait_target_reached_many(self, targets, timeout_s=5.0, *, event_window=None, sta_sequential=False):
        ...
        if sta_sequential:
            ...
            result = {
                ...
                "reached": {f"{int(board)}:{int(motor)}": wait.get("reached", {}).get(f"{int(board)}:{int(motor)}", wait.get("event") or {"source": wait.get("completion_class"), "physical_effect_verified": False})
        ...
        for board, motor in targets:
            key = f"{int(board)}:{int(motor)}"
            event = reached.get(key) if isinstance(reached, dict) else None
            per_axis["x" if int(board) == 5 else "y" if int(board) == 4 else key] = {
                "ok": bool(event), "source_wait_signaled": bool(event),
                "target_reached": bool(event and event.get("status") == 128),
                "event": event if event and event.get("status") == 128 else None,
                "signal": event, "physical_effect_verified": False,
                "failure": None if event else result.get("failure"),
            }
        result["per_axis"] = per_axis
        result["sta_sequential"] = bool(sta_sequential)
        return result
    ...
    def motor_oem_wait_targets_reached(self, targets, timeout_s=5.0, *, event_window=None):
        ...
        deadline = started + max(0.0, float(timeout_s))
        abort_generation = int(getattr(self, "_oem_abort_generation", 0))
        reached = self._take_fallback_motor_signals(targets, event_window)
        first_poll = True
        while reached is None and (first_poll or time.monotonic() <= deadline):
            first_poll = False
            if self.oem_no24v_state() or int(getattr(self, "_oem_abort_generation", 0)) != abort_generation:
                return {"ok": False, "failure": "No24V", "no24v": True, "reached": {}, "events": events}
            events.extend(self._collect_fallback_motor_signals(
                min(0.020, max(0.0, deadline - time.monotonic()))))
            reached = self._take_fallback_motor_signals(targets, event_window)
        return {
            "ok": reached is not None,
            "failure": None if reached is not None else "oem_moveXY_target_event_timeout",
            "pending": [] if reached is not None else [list(key) for key in sorted(targets)],
            "reached": reached or {}, "events": events, "event_window": event_window,
            "elapsed_ms": int((time.monotonic() - started) * 1000),
        }

    def motor_oem_move_absolute(
        self,
        board_id,
        position,
        motor=0,
        *,
        ...
        # The public moveX wrapper owns its <=60 clamp. Direct board calls,
        # including moveXY's parallel branch, pass 0 through to moveToAbs.
        requested = max(0, raw_requested)
        if self.oem_no24v_state():
            raise RuntimeError("Lost 24V power move abs1. moveToAbs()")

        initialized = bool(self._oem_board_state().get(board_id, False))
        if not initialized:
            return {
                "ok": False,
        ...
        if not isinstance(before, dict) or type(current) is not int:
            return {
                "ok": False,
        ...
        try:
            profile = self._motion_oem_axis_profile(axis_key, startup=False)
        except Exception:
            profile = {}
        if not isinstance(profile, dict):
            profile = {}
        min_position = profile.get("axis_min_steps")
        if max_position is None:
            max_position = profile.get("axis_max_steps")
        min_position = int(min_position) if isinstance(min_position, int) else 0
        max_position = int(max_position) if isinstance(max_position, int) else None
        if (
            max_position is not None
            and abs(max_position - int(current)) < 10
            and requested > int(current)
        ):
            return {
                "ok": True,
        ...
        event_window = self.begin_bus_event_window(
            reset_wait_latch=pre_stop.get("wait_latch_reset") is True
        )
        wire_position = max(min_position, requested)
        if max_position is not None:
            wire_position = min(wire_position, max_position)
        time.sleep(0.001)
        home_cache = getattr(self, "_oem_motor_home_cache", None)
        if not isinstance(home_cache, dict):
            home_cache = {}
            self._oem_motor_home_cache = home_cache
        home_cache[(board_id, motor)] = False
        ack = self._send_motor(
            board_id,
            4,
            0,
        ...
        # regardless of the second full TransmitMessage reply. The board
        # wrapper ignores this leaf return and continues with its own wait.
        low_level_source_return_code = (
            0 if ack is None or self._tmcl_success(effective_ack) else 1
        )

        result = {
            "ok": True,
            "source_return_code": 0,
            "low_level_source_return_code": low_level_source_return_code,
            "board": board_id,
            "motor": motor,
            "requested_position": requested,
        ...
        if board_id == self.BOARD_HEAD and motor == 2 and gripper_recover:
            # Head.moveToAbs IL_0093..00ea: software flag only, not rail restore.
            time.sleep(10.0)
            self._oem_24v_dropped = False
            result["gripper_recovery"] = self.motor_oem_axis_search_home(
                "g", speed=600 if self._motion_oem_gripper_version() == 0 else 1500)
            time.sleep(5.0)
            result["y_recovery"] = self.motor_oem_axis_search_home("y", speed=1000)
        if wait_for_stop:
            timeout_s = 30.0 if board_id == self.BOARD_THERMAL else 20.0
            # Recovery runs before the enclosing WaitOne; nested G may consume
            # this same latch. Never reset or replace its cursor afterward.
            ...
            if result["wait"].get("ok") is not True:
                if board_id != self.BOARD_HEAD and (self.oem_no24v_state() or result["wait"].get("failure") == "No24V"):
                ...
                reached_requested = bool(
                    isinstance(timeout_position, dict)
    ...
    def motor_oem_verify_motion_interlock(self):
        """Read-only serial-206 motion preflight for an already prepared profile.

        Exact OEM movement methods assume board/profile setup is complete. This
        check refreshes only the source sensor predicates and never reconnects,
        reactivates boards, toggles the latch, or rewrites motor parameters.
        """
        rail = self.motor_query_24v_sensor()
        door = self.deck_io_query_type(1)
        latch = self.deck_io_query_type(3)
        rail_ok = bool(
            isinstance(rail, dict)
            and self._tmcl_success(rail.get("ack"))
        ...
        door_ok = bool(isinstance(door, dict) and self._tmcl_success(door.get("ack")) and door.get("value") == 1)
        latch_ok = bool(isinstance(latch, dict) and self._tmcl_success(latch.get("ack")) and latch.get("value") == 1)
        snapshot = {"rail_24v": rail, "door": door, "latch": latch}
        return {
            "ok": bool(rail_ok and door_ok and latch_ok),
            "mutation_commanded": False,
            "source": "serial206_query24VSensor+queryDoorSensor+queryLatchSensor",
    ...
    def motor_move_relative(self, board_id, steps, motor=0):
        ...
        pre_stop = self.motor_query_motor_stop(board_id, motor=motor)
        # Board.moveSteps has no extra absolute-move Sleep(1) here.
        ack = self._send_motor(
            int(board_id),
            4,
            1,
        ...
        return {
            ...
            "ack": ack,
            "command_sent": True,
            "controller_acknowledged": acknowledged,
            "source_return_code": 0 if ack is None or acknowledged else 1,
            "ok": True,
        }

    def motor_oem_board_move_steps(self, board, steps, motor=0, *, axis, timeout_s=20.0):
        """Direct board moveSteps: no enclosing CI.getCurrentPosition call."""
        result = self._motor_oem_move_steps_source(
            board=board, motor=motor, axis=axis, steps=steps,
            timeout_s=timeout_s, board_only=True,
        )
        return result

    def _motor_oem_move_steps_source(self, *, board, motor, axis, steps, timeout_s, board_only=False):
        """Recovered board moveSteps plus public moveSteps return semantics."""
        board = int(board)
        motor = int(motor)
        axis = str(axis)
        delta = int(steps)
        board_present = bool(self._oem_board_present(board))
        if not board_present:
            if board_only:
                raise RuntimeError("OEM direct moveSteps board is null")
            return {
                "ok": True,
                "board": board,
                "motor": motor,
                "steps": delta,
                "board_wrapper_return": None,
                "public_wrapper_return": 0,
                "source_return_code": 0,
                "source_call_completed": True,
        ...
        if self.oem_no24v_state():
            raise RuntimeError("Lost 24V power moveSteps1. moveSteps()")

        if not bool(self._oem_board_state().get(board, False)):
            if board_only:
                return {"ok": True, "source_call_completed": True,
                        "board_wrapper_return": 1, "source_return_code": 1,
                        "source_noop": True, "command_sent": False,
                        "source_noop_reason": "board_not_initialized"}
            public_position = self.motor_get_position(board, motor=motor)
            public_value = public_position.get("position") if isinstance(public_position, Mapping) else 0
            ...
            return {
                ...
                "public_wrapper_return": public_value,
                "source_return_code": public_value if source_completed else 0,
                ...
                "command_sent": False,
                "physical_motion_commanded": False,
                "terminal_position": public_position,
            }

        try:
            profile_raw = self._motion_oem_axis_profile(axis, startup=False)
        except TypeError:
            profile_raw = self._motion_oem_axis_profile(axis)
        profile = cast(dict[str, Any], profile_raw) if isinstance(profile_raw, dict) else {}
        if type(profile.get("axis_min_steps")) is not int or type(profile.get("axis_max_steps")) is not int:
            return {
                "ok": False,
                "failure": f"{axis}_source_limits_unavailable",
                "board": board,
                "motor": motor,
                "steps": delta,
                "command_sent": False,
        ...
        low_position = int(profile["axis_min_steps"])
        high_position = int(profile["axis_max_steps"])
        low_margin = low_position + 20
        high_margin = high_position - 20

        cache = getattr(self, "_oem_position_cache", None)
        if not isinstance(cache, dict):
            cache = {}
            self._oem_position_cache = cache
        cached_position = cache.get((board, motor), -1000)
        current = int(cached_position) if type(cached_position) is int else -1000
        before = {"position": current, "source": "ClassMotor.CurrentPosition", "cached": True}
        target = current + delta
        refreshed_before = None

        def beyond_limit(value):
            return int(value) < low_margin or int(value) > high_margin

        if beyond_limit(target):
            refreshed_before = self.motor_get_position(board, motor=motor)
            if not board_only and self.oem_no24v_state():
                raise RuntimeError("Lost 24V power moveSteps2. moveSteps()")
            refreshed_value = refreshed_before.get("position") if isinstance(refreshed_before, Mapping) else None
            if type(refreshed_value) is not int:
                return {
                    "ok": False,
                    "failure": f"{axis}_current_position_unavailable",
                    "board": board,
                    "motor": motor,
                    "steps": delta,
                    "before": before,
            ...
            if beyond_limit(target):
                if board_only:
                    ...
                    return {"ok": True, "source_call_completed": True,
                            "board_wrapper_return": -1, "source_return_code": -1,
                            "source_noop": True, "command_sent": False,
                            "source_noop_reason": "beyond_limit_after_source_refresh"}
                public_position = self.motor_get_position(board, motor=motor)
                public_value = public_position.get("position") if isinstance(public_position, Mapping) else None
                return {
                    "ok": type(public_value) is int,
                    "failure": None if type(public_value) is int else f"{axis}_public_position_unavailable",
                    "board": board,
                    "motor": motor,
                    "steps": delta,
                    "before": before,
                    ...
                    "relative_min_target": low_margin,
                    "relative_max_target": high_margin,
                    "board_wrapper_return": -1,
                    "public_wrapper_return": public_value,
                    "source_return_code": public_value if type(public_value) is int else -1000,
                    "source_call_completed": type(public_value) is int,
                    "source_return_ok": type(public_value) is int,
                    "source_noop": True,
                    "source_noop_reason": "beyond_limit_after_source_refresh",
                    "command_sent": False,
                    "physical_motion_commanded": False,
                    "terminal_position": public_position,
                    "completion_class": "oem_limit_rejected",
                }
        ...
        wait = None
        event = None
        board_wrapper_return = 0
        command_sent = False
        completion_class = "oem_zero_step_noop"
        board_position = None
        if delta != 0:
        ...
        if board_only:
            return {"ok": True, "source_call_completed": True,
                    "board": board, "motor": motor, "steps": delta,
                    "board_wrapper_return": board_wrapper_return,
                    "source_return_code": board_wrapper_return,
                    "command_sent": command_sent, "ack": ack, "wait": wait,
                    "event": event, "board_position": board_position,
        ...
        return {
            "ok": source_completed,
            "failure": None if source_completed else f"{axis}_public_position_unavailable",
            "board": board,
            "motor": motor,
            "steps": delta,
            "before": before,
            ...
            "board_wrapper_return": board_wrapper_return,
            "public_wrapper_return": public_value,
            "source_return_code": public_value if source_completed else -1000,
            "source_call_completed": source_completed,
            "source_return_ok": source_completed,
            "source_noop": delta == 0,
            "source_noop_reason": "zero_steps" if delta == 0 else None,
            "command_sent": command_sent,
            "physical_motion_commanded": command_sent,
            "controller_command_acknowledged": bool(self._tmcl_success(ack)) if command_sent else False,
            "controller_terminal_state_verified": completion_class == "event_128",
            "completion_class": completion_class,
        }

    def motor_y_move_relative_strict(self, steps, *, timeout_s=20.0):
        return self._motor_oem_move_steps_source(
            board=self.BOARD_HEAD, motor=0, axis="y", steps=steps, timeout_s=timeout_s
        )

    def motor_x_move_relative_strict(self, steps, *, timeout_s=20.0):
        return self._motor_oem_move_steps_source(
            board=self.BOARD_DECK, motor=0, axis="x", steps=steps, timeout_s=timeout_s
        )

    def motor_z_move_relative_strict(self, steps, *, timeout_s=20.0):
        return self._motor_oem_move_steps_source(
            board=self.BOARD_HEAD, motor=1, axis="z", steps=steps, timeout_s=timeout_s
        )
    ...
    def motor_move_left(self, board_id, speed=250, motor=0):
        ...
        if not isinstance(home_cache, dict):
            home_cache = {}
            self._oem_motor_home_cache = home_cache
        home_cache[(int(board_id), int(motor))] = False
        ack = self._send_motor(
            int(board_id),
            2,
            0,
    ...
    def motor_query_home_switch(self, board_id, motor=0):
        ...
        reply_valid = bool(self._tmcl_success(row.get("ack")) and type(row.get("value")) is int)
        if reply_valid:
            effective_value = int(row["value"])
            home = effective_value == int(self.MOTOR_SWITCH_ACTIVE_VALUE)
            home_source = "controller_reply"
        elif row.get("ack") is None:
            # ClassMotor.queryLeft returns scalar 0 on a null reply. The OEM
            # queryHome wrapper interprets that helper return as home-active.
            effective_value = 0
            home = True
            home_source = "oem_null_reply_fallback"
        else:
            # A non-null error leaves ClassMotor's initial helper result at 1;
            # the board queryHome wrapper therefore reports not-home.
            effective_value = 1
            home = False
            home_source = "oem_error_sentinel"
        return {
            "board": int(board_id),
            "motor": int(motor),
            "ack": row.get("ack"),
    ...
    def motor_set_home(self, board_id, motor=0):
        # OEM ClassMotor.setHome updates the cached position before SAP1. A null
        # reply returns source scalar 0 before MotorHome becomes true. No GAP
        # readback is part of the recovered method.
        board = int(board_id)
        axis = int(motor)
        channel = (board, axis)
        position_cache = getattr(self, "_oem_position_cache", None)
        if not isinstance(position_cache, dict):
            position_cache = {}
            self._oem_position_cache = position_cache
        position_cache[channel] = 0
        home_cache = getattr(self, "_oem_motor_home_cache", None)
        if not isinstance(home_cache, dict):
            home_cache = {}
            self._oem_motor_home_cache = home_cache
        ...
        if ack is not None:
            home_cache[channel] = True
        source_return_code = 0 if ack is None or direct_ack else 1
        return {
            "board": board,
            "motor": axis,
            "ack": ack,
            ...
            "source_return_code": source_return_code,
            "source_call_completed": True,
            "controller_command_acknowledged": direct_ack,
            "ok": source_return_code == 0,
        }
    ...
    def _motion_oem_gripper_version(self):
        ...
        if not isinstance(machine, dict) or machine.get("ok") is not True:
            raise RuntimeError("immutable OEM machine snapshot is required for GripperVersion")
        raw = (((machine.get("config") or {}).get("config") or {}).get("GripperVersion"))
        try:
            return int(raw)
        except (TypeError, ValueError) as exc:
            raise RuntimeError("serial-206 GripperVersion is missing or malformed") from exc


    def _machine_config_bundle(self):
        """Return only the process-bound immutable OEM machine snapshot projection."""
        if not callable(find_oem_machine_config_bundle):
            return {"ok": False, "blockers": ["find_oem_machine_config_bundle_unavailable"]}
        try:
            return find_oem_machine_config_bundle()
        except Exception as exc:  # pragma: no cover - defensive live path
            return {"ok": False, "blockers": [f"{type(exc).__name__}: {exc}"]}
    ...
    def _machine_config_offset_int(self, machine_key, fallback):
        ...
        if isinstance(machine, dict) and machine.get("ok") is True:
            ...
            if isinstance(offsets, dict) and machine_key in offsets:
                try:
                    return int(offsets[machine_key]), "immutable_oem_machine_snapshot"
                except (TypeError, ValueError):
                    pass
        raise RuntimeError(f"mandatory serial-206 OEM machine field is unavailable: {machine_key}")

    def _machine_config_axis_max(self, axis_key):
        ...
        """
        key = str(axis_key).strip().lower()
        result = self._machine_config_bundle()
        if isinstance(result, dict) and result.get("ok") is True:
            ...
            if isinstance(row, dict) and row.get("max_steps") is not None:
                try:
                    return int(row["max_steps"]), "immutable_oem_machine_snapshot"
                except (TypeError, ValueError):
                    pass
        raise RuntimeError(f"mandatory serial-206 OEM axis limit is unavailable: {key}")
    ...
    def _motion_oem_axis_profile(self, axis_key, *, startup=False):
        ...
        if key == "x":
            x_max, x_max_source = self._machine_config_axis_max("x")
            preset.update({
            ...
            return preset
        if key == "y":
            y_max, y_max_source = self._machine_config_axis_max("y")
            preset.update({
                "speed": preset.get("speed", 1800),
                "acc": preset.get("acc", 400),
                "home_speed": 250 if startup else 500,
                "run_current": 31,
                "standby_current": 20 if startup else preset.get("standby_current", 10),
                "stall_guard": 16,
        ...
        if key == "z":
            ...
            preset.update({
                ...
                "home_speed": 1791,
                "board_test_home_speed": 597,
                "run_current": 31,
                "standby_current": 10,
                "standby_current_written": False,
    ...
    def oem_initialize_without_motion_test_case(self, *, board_wait=None, components=None):
        ...
        selected = {"global", "x", "y", "z", "g", "door"} if components is None else {
            str(component).strip().lower() for component in components
        }
        invalid = selected.difference({"global", "x", "y", "z", "g", "door"})
        if invalid:
        ...
        if report["ok"]:
            ...
            for component in prepared:
                generations[component] = int(generation_value)
                profile = {
                    str(name): value
                    for name, value in dict(self._motion_oem_axis_profile(component)).items()
                }
                fingerprints[component] = {
                    "board": int(profile["board"]),
                    "motor": int(profile["motor"]),
                    "speed": int(profile["speed"]) if type(profile.get("speed")) is int else None,
                    "acceleration": int(profile["acc"]) if type(profile.get("acc")) is int else None,
                    "current": int(profile["run_current"]) if type(profile.get("run_current")) is int else None,
                    "stall_threshold": int(profile["stall_guard"]) if type(profile.get("stall_guard")) is int else None,
                }
    ...
    def motor_oem_require_no_motion_profile(self, axis_key=None, *, expected_overrides=None):
        ...
        # Recovered serial-206 initialization writes SAP12 only for Y and
        # writes no SAP12/SAP13 tuple for X or Z. Keep omitted registers
        # observational rather than turning them into movement admission.
        readbacks = {}
        mismatches = []
        ...
        return {
            ...
            "motor": motor,
            "board_lifecycle_generation": current_generation,
            "profile_fingerprint": dict(
                getattr(self, "_oem_no_motion_profile_fingerprints", {}).get(key, {})
            ),
            "readbacks": readbacks,
        }
    ...
    def motor_oem_axis_search_home(
        ...
        *,
        ...
        OEM sequence:
        setHome(); store search speed; if queryHome() is active, moveToAbs(10000)
        and sleep 500 ms; mark MotorHome false (implicit in Linux); goHome(false).

        Home completion follows the OEM final queryHome()/setHome() semantics.
        Switch transitions and controller readbacks remain telemetry and are not
        success predicates.
        """
        ...
        if self.oem_no24v_state():
            raise RuntimeError(f"Lost 24V power axisSearchHome(). axis: {axis_key}")
        if not bool(self._oem_board_state().get(board, False)):
            return {
                "ok": False,
                "axis": str(axis_key).strip().lower(),
        ...
        # OEM ignores the normal setHome return and queryHome maps a null switch
        # response to active. Neither result is an admission gate.
        sethome_init = self.motor_set_home(board, motor=motor)
        self._oem_store_search_speed(board, motor, speed)
        home_before = self.motor_query_home_switch(board, motor=motor)
        switches_before = None  # axisSearchHome has no diagnostic switch read.
        preclear_move = None
        preclear_wait = None
        home_after_preclear = None
        switches_after_preclear = None
        if home_before.get("home") is True:
            preclear_move = self.motor_oem_move_absolute(
                board,
                10000,
                motor=motor,
                wait_for_stop=True,
            )
            preclear_wait = preclear_move.get("wait") if isinstance(preclear_move, Mapping) else None
            time.sleep(0.50)
        home_cache = getattr(self, "_oem_motor_home_cache", None)
        if not isinstance(home_cache, dict):
            home_cache = {}
            self._oem_motor_home_cache = home_cache
        home_cache[(board, motor)] = False
        go_home = self.motor_oem_go_home(
            axis_key,
            speed=int(speed),
            rehome=False,
            timeout_s=timeout_s,
        ...
        return {
            ...
            "home_after_preclear": home_after_preclear,
            "switches_after_preclear": switches_after_preclear,
            "go_home": go_home,
    ...
    def motor_oem_go_home(
        ...
        *,
        ...
        """Head/Deck goHome, including Head's coupled G timeout recovery."""
        key = str(axis_key).strip().lower()
        if key not in {"x", "y", "z", "g"}:
            return self._motor_oem_go_home_adapted(
                axis_key,
                speed=speed,
                rehome=rehome,
                timeout_s=timeout_s,
        ...
        if not isinstance(preset_raw, Mapping):
            return {"ok": False, "axis": key, "failure": "axis_profile_unavailable", "source_return_code": 1}
        ...
        if self.oem_no24v_state():
            raise RuntimeError("Lost 24V power go home")
        if not bool(self._oem_board_state().get(board, False)):
            return {
                "ok": False,
                "axis": key,
        ...
        # Reuse cached input; source has no pre-search telemetry queries here.
        self._oem_store_search_speed(board, motor, speed)
        position_before = {"position": cached_position_before, "authority": "oem_cached_CurrentPosition"}
        home_before = None
        speed_before = None
        ...
        if bool(rehome):
            ...
            if not isinstance(rehome_move, dict) or (rehome_move.get("ok") is not True
                ...
                return {
                    ...
                    "board": board,
                    "motor": motor,
                    "failure": "rehome_move_source_exception",
                    "source_exception": rehome_move,
                    "source_return_code": None,
                    "rehome_move": rehome_move,
                    "physical_effect_verified": False,
                }
            if board == self.BOARD_DECK:
                deck_rehome_position = self.motor_get_position(board, motor=motor)
                deck_position = deck_rehome_position.get("position") if isinstance(deck_rehome_position, dict) else None
                if type(deck_position) is int:
                    source_return_code = int(deck_position)
            elif type(rehome_move.get("source_return_code")) is int:
                source_return_code = int(rehome_move["source_return_code"])

        event_window = self.begin_bus_event_window()
        move_home = None
        if key == "g" and self.oem_no24v_state():
            raise RuntimeError("Lost 24V power moveLeft 1. moveLeft() axis: g")
        if key != "g" or self._oem_board_state().get(board, False):
            move_home = self.motor_move_left(board, speed=int(speed), motor=motor)
            if key == "g" and self.oem_no24v_state():
                raise RuntimeError("Lost 24V power moveLeft 2. moveLeft() axis: g")
        home_cache[channel] = False
        source_timeout_s = 30.0
        wall_start = time.time() if key == "g" else None
        deadline = time.monotonic() + source_timeout_s
        wait_method = self._motor_oem_gripper_home_wait if key == "g" else self.motor_wait_stopped
        wait = wait_method(
            board,
            motor=motor,
            ...
            min_polls=1,
            oem_board_predicate=True,
            **({"source_wall_start": wall_start, "source_timer_start": deadline - source_timeout_s} if key == "g" else {}),
        )
        if not isinstance(wait, dict) or wait.get("stopped") is not True:
            self.motor_oem_board_stop(board, motor=motor, axis_name=key)
            raise RuntimeError(f"{key} Move to left time out")
        ...
        set_home = None
        position_after = None
        while (time.time() - wall_start < source_timeout_s if wall_start is not None else time.monotonic() < deadline):
            home_poll = ({"home": True, "reply_valid": False, "ack": None,
                          "source_noop": "board_not_initialized"}
                         if key == "g" and not self._oem_board_state().get(board, False)
                         else self.motor_query_home_switch(board, motor=motor))
            home_active = bool(isinstance(home_poll, dict) and home_poll.get("home") is True)
            trace.append({"home": home_poll, "home_active": home_active})
            if home_active:
                home_hit = home_poll
                # Head/Deck.goHome invokes the No24V/initialized board wrapper.
                stop = self.motor_oem_board_stop(board, motor=motor, axis_name=key)
                position_after = self.motor_get_position(board, motor=motor)
                position_value = position_after.get("position") if isinstance(position_after, dict) else position_cache.get(channel, -1000)
                if type(position_value) is int:
                    source_return_code = -int(position_value)
                set_home = self.motor_set_home(board, motor=motor)
                position_cache[channel] = 0
                break

        # Reuse the source poll; never imply a fresh post-setHome observation.
        home_after = home_hit
        ...
        set_home_verified = bool(isinstance(set_home, dict) and self._tmcl_success(set_home.get("ack")))
        home_observed = bool(isinstance(home_hit, dict) and home_hit.get("home") is True
                             and home_hit.get("reply_valid") is True
                             and self._tmcl_success(home_hit.get("ack")))
        return {
            "ok": True,
            "axis": key,
            "board": board,
            ...
            "require_switch_transition_omitted_by_source": bool(require_switch_transition),
            "max_search_abs_delta_omitted_by_source": max_search_abs_delta,
            "position_before": position_before,
            "position_after": position_after,
            "position_after_sethome": position_after_sethome,
            "home_before": home_before,
            "home_hit": home_hit,
            "home_after": home_after,
            "rehome_move": rehome_move,
            "deck_rehome_position": deck_rehome_position,
            "move_home": move_home,
            "move_left": move_home,
            "event_window": event_window,
            "wait": wait,
            "stop": stop,
            "set_home": set_home,
            "controller_command_acknowledged": bool(isinstance(move_home, dict) and self._tmcl_success(move_home.get("ack"))),
            "controller_terminal_state_verified": terminal_zero_verified,
            "controller_home_proof_verified": bool(terminal_zero_verified and home_observed and set_home_verified),
            "home_decision": {
                "source_returned_normally": True,
                "home_observed": home_observed,
                "terminal_stop_verified": stop_acknowledged and terminal_zero_verified,
                "set_home_verified": set_home_verified,
            },
            "source_return_code": int(source_return_code),
            "trace_tail": trace[-40:],
            "independent_physical_motion_verified": False,
            "physical_effect_verified": False,
        }

    def _oem_store_search_speed(self, board, motor, speed):
        speeds = getattr(self, "_oem_search_home_speeds", None)
        if speeds is None:
            speeds = self._oem_search_home_speeds = {}
        speeds[(int(board), int(motor))] = int(speed)

    def _motor_oem_gripper_recovery_position(self):
        from .oem_initialization import build_machine_calibration_manifest
        manifest = build_machine_calibration_manifest()
        if not manifest.get("ok"):
            raise RuntimeError("OEM gripper recovery settings unavailable")
        values = manifest["gripper"]
        offset = values["originOffsetG"]["value"]
        wide = values["GripperOpenWide"]["value"]
        version = self._motion_oem_gripper_version()
        if version == 0 and type(offset) is int:
            return 59500 + offset
        if version == 1 and type(wide) is int:
            if wide != 0:
                return wide
            if type(offset) is int:
                return 33000 + offset
        raise RuntimeError("OEM GripperStatus4 binding unavailable")

    def _motor_oem_gripper_home_wait(self, board, *, motor, source_wall_start, source_timer_start, **_):
        # Predicate precedes timeout. The first recovery stops the stopwatch;
        # neither it nor the DateTime origin restarts on subsequent recovery.
        wall_start, timer_start = source_wall_start, source_timer_start
        stopped_elapsed = None
        polls = 0
        while True:
            no24v = self.oem_no24v_state()
            if no24v or not self._oem_board_state().get(int(board), False):
                return {"stopped": True, "source_predicate_reason": "No24V" if no24v else "board_not_initialized",
                    "controller_terminal_state_verified": False, "speed_reply_valid": False,
                    "last_speed": None, "last_ack": None, "polls": polls}
            row = self.motor_get_speed(board, motor=motor)
            polls += 1
            if row.get("speed") == 0:
                valid = row.get("speed_reply_valid") is True and self._tmcl_success(row.get("ack"))
                return {"stopped": True, "last_speed": 0, "last_ack": row.get("ack"),
                    "speed_reply_valid": valid, "controller_terminal_state_verified": valid, "polls": polls}
            elapsed = time.monotonic() - timer_start if stopped_elapsed is None else stopped_elapsed
            if time.time() - wall_start > 30.0 and elapsed > 30.0:
                stopped_elapsed = elapsed
                self.motor_oem_move_absolute(board, self._motor_oem_gripper_recovery_position(),
                    motor=motor, wait_for_stop=True, gripper_recover=True)

    def _motor_oem_go_home_adapted(
        self,
        axis_key,
        *,
        speed,
        ...
        """Source-faithful Class*Board.goHome(rehome, axis, speed), switch-driven.

        This intentionally uses OEM MoveLeft/cmd=2 for all axes. The search is stopped
        by polling queryHome/GAP9 during motion; after stop, setHome() re-establishes
        controller coordinate zero.  Inactive->active transition is recorded as data,
        not required as a Linux-only success predicate.
        """
        ...
        home_after_rehome = None
        switches_after_rehome = None
        rehome_exception = None
        if bool(rehome):
            try:
                rehome_move = self.motor_move_absolute(board, 10000, motor=motor)
                rehome_wait = self.motor_wait_stopped(
                    board,
                    motor=motor,
                ...
                home_after_rehome = self.motor_query_home_switch(board, motor=motor)
                switches_after_rehome = self.motor_get_switch_activity(board, motor=motor)
            except Exception as exc:
                rehome_exception = f"{type(exc).__name__}: {exc}"
        search_preflight_exception = None
        try:
            search_start_position = self.motor_get_position(board, motor=motor)
            search_start_home = self.motor_query_home_switch(board, motor=motor)
        except Exception as exc:
            search_preflight_exception = f"{type(exc).__name__}: {exc}"
            search_start_position = {"error": search_preflight_exception}
            search_start_home = {"error": search_preflight_exception}
        search_start_value = search_start_position.get("position") if isinstance(search_start_position, dict) else None
        axis_min_steps = preset.get("axis_min_steps")
        axis_max_steps = preset.get("axis_max_steps")
        pre_home_outside_axis_limit = False
        if search_start_value is not None:
            if axis_min_steps is not None and int(search_start_value) < int(axis_min_steps):
                pre_home_outside_axis_limit = True
            if axis_max_steps is not None and int(search_start_value) > int(axis_max_steps):
                pre_home_outside_axis_limit = True
        ...
        home_hit = None
        trace = []
        controller_speed_nonzero_seen = False
        controller_position_counter_changed = False
        seen_inactive = search_start_home.get("value") != active_value
        failure = None
        wait = None
        move_home = None
        ...
        try:
            while failure is None and time.monotonic() < deadline:
                if getattr(self, "_oem_active_board_lifecycle_generation", None) != board_lifecycle_generation:
                    failure = "board_lifecycle_generation_changed_during_home"
                    break
                now = time.monotonic()
                home_poll = self.motor_query_home_switch(board, motor=motor)
                speed_poll = self.motor_get_speed(board, motor=motor)
                position_poll = self.motor_get_position(board, motor=motor)
                switches_poll = self.motor_get_switches(board, motor=motor)
                ...
                if (
                    search_start_value is not None
                    and current_position is not None
                    and int(current_position) != int(search_start_value)
                ):
                    controller_position_counter_changed = True
                controller_motion_evidence_seen = bool(
                    controller_speed_nonzero_seen or controller_position_counter_changed
                )
                if home_value != active_value:
                    seen_inactive = True
                row = {
                    ...
                    "left": switches_poll.get("left_state") if isinstance(switches_poll, dict) else None,
                    "right": switches_poll.get("right_state") if isinstance(switches_poll, dict) else None,
                }
                trace.append(row)
                if home_value == active_value:
                    home_hit = home_poll
                ...
                if (
                    ...
                    and search_start_value is not None
                    and current_position is not None
                    and abs(int(current_position) - int(search_start_value)) >= stop_threshold
                ):
                    failure = "max_search_stop_margin_reached_before_home_switch_transition"
                    break
                time.sleep(0.05)
        except Exception as exc:
            failure = failure or f"home_monitor_exception:{type(exc).__name__}:{exc}"

        # Always issue a bounded stop after a search attempt or failed preflight.
        # No coordinate rebase is permitted until every stop/reference predicate is
        ...
        try:
            wait = self.motor_wait_stopped(board, motor=motor, timeout_s=3.0, require_seen_nonzero=False)
        except Exception as exc:
            wait = {"stopped": False, "error": f"{type(exc).__name__}: {exc}"}
        try:
            position_after = self.motor_get_position(board, motor=motor)
            home_after_stop = self.motor_query_home_switch(board, motor=motor)
            source_events = self.collect_bus_events(duration_s=0.25, timeout_ms=12, max_events=96)
        except Exception as exc:
            position_after = {"error": f"{type(exc).__name__}: {exc}"}
        ...
        ok = failure is None
        source_position = position_after.get("position") if isinstance(position_after, dict) else None
        source_return_code = -int(source_position) if ok and isinstance(source_position, int) else 1
        return {
            "axis": key,
            "board": board,
            "motor": motor,
            ...
            "max_search_abs_delta": None if max_search_abs_delta is None else int(max_search_abs_delta),
            "search_stop_margin_steps": stopping_margin,
            "search_stop_threshold_steps": stop_threshold,
            "axis_min_steps": None if axis_min_steps is None else int(axis_min_steps),
            "axis_max_steps": None if axis_max_steps is None else int(axis_max_steps),
            "pre_home_outside_axis_limit": bool(pre_home_outside_axis_limit),
            "search_start_home": search_start_home,
            "home_hit": home_hit,
            "home_after_stop": home_after_stop,
            "home_after": home_after,
            "rehome_move": rehome_move,
            "rehome_wait": rehome_wait,
            "rehome_position": rehome_position,
            "move_home": move_home,
            "move_left": move_home,
            ...
            "home_predicate_confirmed": bool(ok),
            "false_home_guard": failure,
            "source_return_code": source_return_code,
            "trace_tail": trace[-40:],
            "ok": ok,
    ...
    def motor_oem_confirm_thermal_door_closed(self) -> dict:
        ...
        if not isinstance(preset, dict):
            raise RuntimeError("thermal door profile unavailable")
        home = self.motor_query_home_switch(int(preset["board"]), motor=int(preset["motor"]))
        if not isinstance(home, Mapping) or type(home.get("value")) is not int:
            raise RuntimeError("tcDoorClosed_source_predicate_unavailable")
        closed = home["value"] == int(self.MOTOR_SWITCH_ACTIVE_VALUE)
        return {"home": home, "oem_predicates": {
            "tcDoorClosed": closed, "closed_source": "queryHome(ThermalDoor)"}}
    ...
    def _motor_oem_door_search_wait(self, board, motor):
        ...
        The postdecrement tests the OLD counter. Even an already-stopped motor
        gets queryHome and Sleep(50) first. Cached scalar speed zero remains a
        source predicate only, never fresh stopped/physical evidence.
        """
        counter = 300
        trace = []
        last = None
        while True:
            home = self._motor_oem_door_query_home(board, motor)
            if isinstance(home, dict) and home.get("home") is True and counter > 81:
                counter = 80
            time.sleep(0.05)
            old_counter = counter
            counter -= 1
            row = {"home": home, "counter_before_decrement": old_counter}
            trace.append(row)
            if old_counter < 0:
                return {"stopped": False, "timeout": True, "source_counter": counter,
                        "polls": len(trace) - 1, "trace": trace,
                        "last_speed": last.get("speed") if isinstance(last, dict) else None,
                        "last_ack": last.get("ack") if isinstance(last, dict) else None,
                        "controller_terminal_state_verified": False}
            # Thermal.checkMotorStopped: No24V, initialized, then one source
            # queryMotorSpeed (its null-reply leaf retry is not an outer poll).
            no24v = self.oem_no24v_state()
            if no24v or not self._oem_board_state().get(int(board), False):
                return {"stopped": True, "source_counter": counter, "trace": trace,
                        "polls": len(trace) - 1,
                        "source_predicate_reason": "No24V" if no24v else "board_not_initialized",
                        "controller_terminal_state_verified": False,
                        "speed_reply_valid": False, "last_speed": None, "last_ack": None}
            last = self.motor_get_speed(board, motor=motor)
            row["speed"] = last
            if isinstance(last, dict) and last.get("speed") == 0:
                valid = bool(last.get("speed_reply_valid") is True
                             and self._tmcl_success(last.get("ack")))
                return {"stopped": True, "source_counter": counter, "trace": trace,
                        "polls": len(trace), "last_speed": last.get("speed"),
                        "last_ack": last.get("ack"), "speed_reply_valid": valid,
                        "controller_terminal_state_verified": valid}

    def _motor_oem_door_source_settings(self):
        # CI constructor IL_02d6..02ec binds ThermalDoor to ZLow/HighLimit.
        # Branch selectors must come from the same accepted immutable snapshot,
        # never a serial206/default CameraCalibrated guess.
        from .oem_machine_bundle import get_active_oem_machine_snapshot
        snapshot = get_active_oem_machine_snapshot()
        return {"serial": int(snapshot.machine_serial),
                "camera_calibrated": snapshot.camera_calibrated,
                "min_steps": int(snapshot.axis_limits["z"]["min_steps"]),
                "max_steps": int(snapshot.axis_limits["z"]["max_steps"])}

    def _motor_oem_door_query_home(self, board, motor):
        # Thermal.queryHome returns true without querying an uninitialized board.
        if not self._oem_board_state().get(int(board), False):
            return {"home": True, "reply_valid": False, "ack": None,
                    "home_source": "board_not_initialized"}
        return self.motor_query_home_switch(board, motor=motor)

    def _motor_oem_door_preclear(self, board, motor, settings):
        """Thermal.moveSteps(axis, 2000, true), without CI's extra query.

        ClassCanLib RVAab38: cached bound, optional refresh, query138/relative,
        30000ms AutoResetEvent wait, one final position query then power check.
        A normal -1 return does not prevent doorSearchHome's continuation.
        """
        if self.oem_no24v_state():
            raise RuntimeError("Lost 24V power moveSteps1. moveSteps() axis: door")
        if not self._oem_board_state().get(int(board), False):
            return {"source_return_code": 1, "source_noop": "board_not_initialized"}
        cache = getattr(self, "_oem_position_cache", {})
        current = cache.get((int(board), int(motor)), -1000)
        def beyond(position):
            return position > settings["max_steps"] - 20 or position < settings["min_steps"] + 20
        before = None
        if beyond(current + 2000):
            before = self.motor_get_position(board, motor=motor)
            current = before["position"]
            if beyond(current + 2000):
                if self.oem_no24v_state():
                    raise RuntimeError("Lost 24V power moveSteps2. moveSteps() axis: door")
                return {"source_return_code": -1, "source_noop": "beyond_limit_after_source_refresh",
                        "before": before}
        pre_stop = self.motor_query_motor_stop(board, motor=motor)
        # Cursor BEFORE issuance: an event arriving before its ACK is valid.
        event_window = self.begin_bus_event_window(
            reset_wait_latch=pre_stop.get("wait_latch_reset") is True)
        home_cache = getattr(self, "_oem_motor_home_cache", None)
        if not isinstance(home_cache, dict):
            home_cache = {}
            self._oem_motor_home_cache = home_cache
        home_cache[(int(board), int(motor))] = False
        ack = self._send_motor(board, 4, 1, motor, 2000,
            attempts=1, wait_reply=True, write_timeout_ms=55,
            read_timeout_ms=60000, max_reads=1, strict_match=True,
            allow_recover=False, ordinary_motor_retry=True)
        move = {"pre_stop": pre_stop, "ack": ack}
        wait = self.motor_oem_wait_target_reached(
            board, motor=motor, timeout_s=30.0, event_window=event_window)
        position = self.motor_get_position(board, motor=motor)
        completed = isinstance(wait, dict) and wait.get("ok") is True
        if self.oem_no24v_state():
            stage = 4 if completed else 3
            raise RuntimeError(f"Lost 24V power moveSteps{stage}. moveSteps() axis: door")
        return {"source_return_code": position["position"] if completed else -1,
                "before": before, "move": move, "wait": wait, "position": position,
                "physical_effect_verified": False}

    def motor_oem_door_search_home(self, *, timeout_s=20.0, startup=False):
        """Thermal.doorSearchHome: source return is not physical proof.

        Both CI HomeAxis(D) and startup use the same board method. Its counter
        schedule and preclear event budget do not depend on the caller timeout.
        """
        preset = cast(dict[str, Any], self._motion_oem_axis_profile("door"))
        board, motor = int(preset["board"]), int(preset["motor"])
        if self.oem_no24v_state():
            raise RuntimeError("Lost 24V power doorSearchHome() axis: door")
        if not self._oem_board_state().get(board, False):
            return {"ok": True, "axis": "door", "board": board, "motor": motor,
                    ...
                    "controller_home_proof_verified": False,
                    "physical_effect_verified": False}
        settings = self._motor_oem_door_source_settings()
        threshold = int(preset["stall_guard"])
        speed = int(preset["home_speed"])
        home_before = self._motor_oem_door_query_home(board, motor)
        preclear_threshold = preclear_move = None
        if home_before.get("home") is True or settings["serial"] < 10:
            preclear_threshold = self.motor_set_axis_param(board, 205, threshold + 2, motor=motor)
            preclear_move = self._motor_oem_door_preclear(board, motor, settings)
        threshold_restore = self.motor_set_axis_param(board, 205, threshold, motor=motor)
        # Thermal.moveLeft board wrapper: No24V -> initialized -> leaf -> No24V.
        if self.oem_no24v_state():
            raise RuntimeError("Lost 24V power moveLeft 1. moveLeft() axis: door")
        move_left = None
        if self._oem_board_state().get(board, False):
            move_left = self.motor_move_left(board, speed=speed, motor=motor)
            if self.oem_no24v_state():
                raise RuntimeError("Lost 24V power moveLeft 2. moveLeft() axis: door")
        wait = self._motor_oem_door_search_wait(board, motor)
        # Thermal.stopMotor ignores waitforstop; the existing board wrapper is
        # already source-correct and remains unchanged.
        ...
        return {"ok": True, "axis": "door", "board": board, "motor": motor,
                ...
                "closed_after": home_after.get("home"), "opened_after": None,
                "closed_confirmed": home_observed,
                "preclear_threshold": preclear_threshold, "preclear_move": preclear_move,
    ...
    def motor_oem_switch_search_home_axis(self, axis_key, *, speed=None, timeout_s=20.0):
        ...
        This intentionally does not use the Z startup fast path that accepts the
        live GAP10/right-at-controller-zero predicate without motion. Manual Home
        must either prove a switch transition before setHome(), or return ok=false.
        """
        preset = self._motion_oem_axis_profile(axis_key, startup=True)
    ...
    def _motor_oem_home_gripper_scoped(
        ...
        *,
        ...
        """Home-only startup delegation; standalone G keeps its OEM lifetime."""
        if not startup:
            board, motor = int(preset["board"]), int(preset["motor"])
            if not self._oem_board_present(board):
                return {"axis": "g", "source_noop": "board_null", "home": {"ok": True, "source_noop": True}}
            version = int(preset["gripper_version"])
            prepare = self.motor_set_axis_param(board, 6, 31, motor=motor)
            home = self.motor_oem_go_home("g", speed=600 if version == 0 else 200,
                rehome=True, timeout_s=timeout_s)
            restore = self.motor_set_axis_param(board, 6, 10, motor=motor) if version == 1 else None
            return {"axis": "g", "startup": False, "prepare": prepare,
                    "home": home, "restore_current": restore, "source_current_retained": version != 1}
        # Startup home-only role: the enclosing initializeMotors owns current
        # 31, moveSteps(+10000,true), and final current10. Manual branch above
        # retains its separate source contract. Legacy current flags cannot
        # cause additional startup writes or finally restoration.
        effective_speed = int(preset.get("home_speed", 200)) if speed is None else int(speed)
        home = self.motor_oem_axis_search_home(
            "g", speed=effective_speed, timeout_s=timeout_s,
            max_search_abs_delta=preset.get("home_search_max_abs_delta"),
        )
        return {"axis": "g", "startup": True, "prepare": None,
                "home": home, "restore_current": None,
                "current_lifetime_owner": "ClassControlInterface.initializeMotors"}

    def motor_oem_move_z_home(self, *, rehome=True, timeout_s=30.0):
        """Literal ``ClassControlInterface.MoveZHome`` for serial-206 Z."""
        preset = dict(self._motion_oem_axis_profile("z", startup=False))
        board = int(preset["board"])
        motor = int(preset["motor"])
        if not self._oem_board_present(board):
            return {
        ...
        set_current = self.motor_set_axis_param(board, 6, current, motor=motor)
        read_current = self.motor_get_axis_param(board, 6, motor=motor)
        current_ok = bool(
            isinstance(set_current, dict)
            and set_current.get("ok") is True
            and isinstance(read_current, dict)
            and read_current.get("value") == current
        )
        home = None
        if current_ok:
        ...
        return {
            ...
            "axis": "z",
            "oem_method": "ClassControlInterface.MoveZHome",
            "source_anchor": "ClassControlInterface.cs:4623-4632",
            "profile": profile,
            "set_z_current_param6": set_current,
            "read_z_current_param6": read_current,
    ...
    def motor_oem_home_gz(
        ...
        *,
        ...
        if not z_present:
            home_g = self.motor_oem_home_axis_board_test("g", timeout_s=timeout_s)
            return {
                "ok": bool(home_g.get("ok")),
                "branch": "z_board_null_home_gripper",
                "home_g": home_g,
                "source_anchor": "ClassControlInterface.homeGZ:4657-4663",
            }
        if not g_present:
            z_home = self.motor_oem_move_z_home(rehome=True, timeout_s=timeout_s)
            return {
                "ok": bool(z_home.get("ok")),
                "branch": "gripper_board_null_move_z_home",
                "z_home": z_home,
                "source_anchor": "ClassControlInterface.homeGZ:4664-4667",
            }

        z_profile = self._motion_oem_axis_profile("z", startup=False)
        g_profile = self._motion_oem_axis_profile("g", startup=False)
        set_gripper_current = self.motor_set_axis_param(
            g_profile["board"], 6, 31, motor=g_profile.get("motor", 0)
        )
        z_move = self.motor_set_axis_param(
            z_profile["board"], 6, 31, motor=z_profile.get("motor", 0)
        )
        z_move_result = self.motor_oem_move_absolute(
            z_profile["board"],
            max(int(pseudo_z_home), 0),
            motor=z_profile.get("motor", 0),
            wait_for_stop=False,
            max_position=z_profile.get("axis_max_steps"),
        )
        time.sleep(max(0.0, int(delay_s) * 1.0))
        if gripper_version is None:
            gripper_version = self._motion_oem_gripper_version()
        if type(development_machine) is not bool:
            raise RuntimeError("authoritative development-machine state is required for homeGZ")
        speed = 200 if int(gripper_version) != 0 else 600
        gripper_home = self.motor_oem_go_home(
            "g",
            speed=speed,
            rehome=False,
            timeout_s=float(timeout_s),
        ...
        if caught_plate and not bool(development_machine):
            ...
            # Source latchStatus assignment follows the normal void call,
            # independent of ACK; this is host state, not an unlocked sensor.
            self._mark_oem_latch_unlocked()
            recovery = {
                "x_home": x_home,
                "solenoid": solenoid,
        ...
        # CI.moveZ always sets current, including its second homeGZ call.
        z_restore_current = self.motor_set_axis_param(
            z_profile["board"], 6, 31, motor=z_profile.get("motor", 0))
        ...
        return {
            ...
            "caught_plate": False,
            "z_restore": z_restore,
            "z_restore_current": z_restore_current,
    ...
    def motor_oem_home_axis(
        ...
        *,
        ...
        if not isinstance(preset_raw, dict):
            raise RuntimeError(f"invalid OEM motion profile for {axis_key_norm!r}")
        preset = {str(key): value for key, value in preset_raw.items()}
        if axis_key_norm == "g":
            return self._motor_oem_home_gripper_scoped(
                preset,
                speed=speed,
                timeout_s=timeout_s,
    ...
    def motor_wait_stopped(
        ...
        last = None
        last_speed = None
        last_speed_reply_valid = False
        seen_nonzero = False
        last_position = None
        target_reached = False
        while time.monotonic() < deadline:
            # Head/Deck.checkMotorStopped checks No24V BEFORE initialized and
            # returns true without telemetry in either short-circuit branch.
            # This is source control flow, never controller stopped evidence.
            if oem_board_predicate:
                no24v = self.oem_no24v_state()
                if no24v or not self._oem_board_state().get(int(board_id), False):
                    return {"stopped": True, "source_predicate_reason": "No24V" if no24v else "board_not_initialized",
                            "controller_terminal_state_verified": False, "speed_reply_valid": False,
                            "last_speed": last_speed, "last_ack": last, "polls": polls,
                            "elapsed_ms": int((time.monotonic() - t0) * 1000)}
            row = self.motor_get_speed(board_id, motor=motor)
            polls += 1
            last = row.get("ack")
            last_speed = row.get("speed")
            last_speed_reply_valid = bool(row.get("speed_reply_valid") is True and self._tmcl_success(last))
            ...
            if (
                ...
                if (
                    ...
                    and type(position_row.get("position")) is int
                ):
            ...
            if (
                ...
                return {
                    ...
                    "last_ack": last,
                    "speed_reply_valid": last_speed_reply_valid,
                    "controller_terminal_state_verified": last_speed_reply_valid and type(last_speed) is int,
                }
            if not oem_board_predicate:
                time.sleep(max(0.02, float(poll_s)))
        return {
            "stopped": False,
            "controller_terminal_state_verified": False,
    ...
    def motor_oem_stop_exact(self, board_id, motor=0):
        ...
        source_return_code = 0 if second is None or second_ok else 1
        return {
            "board": int(board_id),
            "motor": int(motor),
            "ack": second,
    ...
    def motor_oem_board_stop(self, board_id, motor=0, *, axis_name=None):
        axis = str(axis_name or self.motor_axis_key_for_channel(board_id, motor=motor))
        if self.oem_no24v_state():
            raise RuntimeError(f"Lost 24V power stopMotor 1. stopMotor() axis: {axis}")
        if not self._oem_board_state().get(int(board_id), False):
            return {
                "ok": True,
                "board": int(board_id),
                "motor": int(motor),
        ...
        result = self.motor_oem_stop_exact(board_id, motor=motor)
        if self.oem_no24v_state():
            raise RuntimeError(f"Lost 24V power stopMotor2. stopMotor() axis: {axis}")
        # Board stopMotor is void (ClassHeadBoard IL_0034 pop). Preserve the
        # leaf scalar/ACK fields separately; normal board return is not proof.
        return {**result, "source_call_completed": True, "source_board_return": None}
