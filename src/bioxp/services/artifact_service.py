...
DEFAULT_VALIDATION_ARTIFACT_ROOT = Path.home() / ".local" / "share" / "bioxp" / "motion_validation"
...
def _validation_artifact_root_candidates(primary_root: Path) -> list[Path]:
    """Return primary root first, then a robot-local writable fallback."""
    fallback = Path.home() / ".local" / "share" / "bioxp" / "motion_validation"
    candidates = [primary_root]
    if fallback != primary_root:
...
def create_motion_validation_bundle(
    *,
    ...
    requested_root = get_validation_artifact_root()
    bundle_id = (
    ...
    for root in _validation_artifact_root_candidates(requested_root):
        ...
        root_fallback = root != requested_root
        ...
        bundle_summary = {
            ...
            "root_fallback_reason": None if not root_fallback else f"primary root unavailable: {last_error}",
    ...
    raise last_error or OSError("motion validation artifact root unavailable")
