...
class OfflineSwitchAuditFixture:
    """Offline fixture for tests/docs only; never proof of live robot state."""
...
IMPLEMENTATION_MAPPED_HOME_PREDICATES = {
    # Implementation-mapped polarity: ClassMotor.queryLeftSwitchStatus/queryRightSwitchStatus
    # read GAP9/GAP10 and return code 0 when the raw reply value is 1; board-level
    # queryHome/queryRightSensor treat return-code 0 as True. Linux exposes the raw
    # GAP value, so active_value is raw 1. This remains blocked for live homing until
    # the unit-specific physical direction/channel is verified with a supervised trace.
    "x": OemHomePredicate(axis="x", home_switch="gap9_left", active_value=1, search_direction="move_left", confidence="implementation_mapped", source="ClassMotor.queryLeftSwitchStatus GAP9 raw 1 -> return 0; ClassBoard.queryHome true on return 0; axisSearchHome/goHome use MoveLeft"),
    "y": OemHomePredicate(axis="y", home_switch="gap9_left", active_value=1, search_direction="move_left", confidence="implementation_mapped", source="ClassMotor.queryLeftSwitchStatus GAP9 raw 1 -> return 0; ClassBoard.queryHome true on return 0; axisSearchHome/goHome use MoveLeft"),
    "z": OemHomePredicate(axis="z", home_switch="gap9_left", active_value=1, search_direction="move_left", confidence="implementation_mapped", source="ClassMotor.queryLeftSwitchStatus GAP9 raw 1 -> return 0; ClassHeadBoard.queryHome true on return 0; MotorZ.axisSearchHome/goHome use MoveLeft"),
    "g": OemHomePredicate(axis="g", home_switch="gap9_left", active_value=1, search_direction="move_left", confidence="implementation_mapped", source="ClassMotor.queryLeftSwitchStatus GAP9 raw 1 -> return 0; ClassHeadBoard.queryHome true on return 0; MotorGrip.axisSearchHome/goHome use MoveLeft"),
    "door": OemHomePredicate(axis="door", home_switch="gap9_left", active_value=1, search_direction="move_left", confidence="implementation_mapped", source="ClassMotor.queryLeftSwitchStatus GAP9 raw 1 -> return 0; ClassBoard.queryHome true on return 0; doorSearchHome uses MoveLeft"),
}
...
def interpret_home_predicate(axis: str, snap: dict) -> dict:
    ...
    if axis == "z":
        payload["blockers"].extend([
            "z_live_homing_trace_required_before_live_verified_confidence",
