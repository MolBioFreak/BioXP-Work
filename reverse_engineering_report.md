...
* **Separation of Concerns and API Bridging:** By replacing the legacy Windows OS on the existing internal compute node (a 2018-vintage 1C/2T AMD processor with 4GB DDR3) with an efficient Linux distribution (like Arch Linux or Ubuntu, utilizing a lightweight desktop or Wayland compositor), we can construct a pure `python-can` interface while maintaining physical touchscreen capabilities. The node will still act as a headless endpoint to the rest of the lab via REST/FastAPI, while rendering a local UI specifically for on-machine interactions. 
...
1. **The Native Kiosk Approach (Direct Drive):** Since the AMD chip previously ran Windows 10 Enterprise + a WPF application (which offloads heavy UI rendering to the integrated GPU via DirectX), we can achieve similar graphical efficiency on Linux. Configure the OS to boot into a minimal Wayland compositor (like Sway or Weston) or a very lightweight desktop environment (XFCE / LXQt) running a web browser mapping to `http://localhost:8000`. This preserves the exact same built-in touch experience on the physical display without the overhead of heavy Linux desktops like GNOME.
...
2.  **The "Drop-In" Linux Replacement:**
    *   You wipe the Windows OS completely off the 120GB SATA SSD.
    *   You install an efficient Linux distribution (e.g., Ubuntu, Arch Linux) directly onto the legacy AMD motherboard, configured with a minimal GUI (to respect the 4GB RAM footprint).
    *   **The internal USB connections to the CAN adapter remain completely untouched.**
...
> **The "Low Pain" Gameplan:** Do not try to convert the proprietary `.NET 4.5.2` app into a modern .NET 8 cross-platform app. Instead, treat the decompiled DLLs as a **Rosetta Stone**. You decompile `BioXPControlLib.dll` once to read exactly how it formats a `Dispense` or `MoveZ` command into an 8-byte payload. Then, you wipe the 120GB SSD, install an efficient Linux OS, and write a fresh, ultra-lean **Python** script using `python-can`. You broadcast those exact byte arrays natively through Linux's unified `SocketCAN` interface. This eliminates the massive `.NET` / unneeded Windows Enterprise background overhead, freeing up your 1C/2T AMD chip to easily run an efficient FastAPI Tailnet hub and a local lightweight touchscreen UI simultaneously.
