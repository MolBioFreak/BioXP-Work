...
* **Separation of Concerns and API Bridging:** By replacing the legacy Windows OS on the existing internal compute node (a 2018-vintage 1C/2T AMD processor with 4GB DDR3) with a headless Linux distribution, we can construct a pure `python-can` interface. Because this hardware is severely resource-constrained, running a headless Microservice (like FastAPI or gRPC) is the *only* performant way to expose the robot's capabilities without choking the 1-core processor. 
...
1. **The Native Kiosk Approach (Direct Drive):** You can connect a modern, capacitive HDMI/USB touchscreen directly to the existing AMD motherboard. You simply configure the Linux OS to boot directly into a "kiosk mode" web browser (like Chromium) pointing to `http://localhost:8000`. This provides the exact same "built-in" feel as the original instrument, but powered by your new, lightweight React/Vue web UI. Note: Chromium may consume significant RAM, so this must be monitored against the 4GB limit.
...
2.  **The "Drop-In" Linux Replacement:**
    ...
    *   You install a barebones, headless Linux server distribution (e.g., Ubuntu Server or Alpine Linux) directly onto the legacy AMD motherboard to maximize the 4GB RAM budget.
