...
1. **Headless Ubuntu 24.04 Integration:** Given the constrained 1C/2T AMD processor with 4GB of RAM, the most robust path is to entirely forego the local GUI. Disconnect or ignore the physical touchscreen and rely exclusively on a headless Ubuntu Server 24.04 installation. The physical interaction shifts entirely to your networked lab dashboard or a portable tablet communicating over the Tailnet infrastructure.
...
2.  **The "Drop-In" Linux Replacement:**
    ...
    *   You install headless Ubuntu Server 24.04 directly onto the legacy AMD motherboard, preserving all 4GB of RAM purely for the `python-can` and FastAPI background services.
